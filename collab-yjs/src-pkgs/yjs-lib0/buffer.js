 ...
 * @param {ArrayBuffer} buffer
 */
export const createUint8ArrayFromArrayBuffer = (buffer) =>
 ...
 */
export const toBase64UrlEncoded = (buf) =>
  toBase64(buf).replaceAll('+', '-').replaceAll('/', '_').replaceAll('=', '');

/**
 * @param {string} base64
 */
export const fromBase64UrlEncoded = (base64) =>
 ...
 * @param {Uint8Array} buf
 */
export const toHexString = (buf) =>
  array.map(buf, (b) => b.toString(16).padStart(2, '0')).join('');
...
export const fromHexString = (hex) => {
  ...
  for (let i = 0; i < hlen; i += 2) {
    buf[buf.length - i / 2 - 1] = Number.parseInt(
      hex.slice(hlen - i - 2, hlen - i),
      16,
    );
  }
  return buf;
