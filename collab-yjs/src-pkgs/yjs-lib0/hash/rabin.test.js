...
export const testIrreducibleSpread = (_tc) => {
  ...
  t.info(
    `Average spread for degree ${degree} at ${N} repetitions: ${avgSpread}`,
  );
  t.assert(
    diffSpread < 4,
    'Spread of irreducible polynomials is within expected range',
  );
};
...
const _testFingerprintCompatiblityK = (tc, K) => {
  ...
  t.measureTime('polynomial direct', () => {
    fingerprints1 = dataObjects.map((o, _index) =>
      gf2.fingerprint(o, irreducible),
    );
  });
  ...
  let fingerprints4 = [];
  t.measureTime(
    'polynomial incremental (efficent & cached)) using encoder',
    () => {
...
export const testConflicts = (tc) => {
  ...
  const conflicts = array.fold(
    map.map(results, (ds) => ds.size - 1),
    0,
    math.add,
  );
