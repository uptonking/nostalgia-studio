...
export const toUint8Array = (
  p,
  byteLength = _degreeToMinByteLength(getHighestDegree(p)),
) => {
 ...
 * @return number
 */
export const getHighestDegree = (p) =>
  array.fold(array.from(p.degrees), 0, math.max);
 ...
 * @param {GF2Polynomial} m
 */
export const fingerprint = (buf, m) =>
  toUint8Array(
    mod(createFromBytes(buf), m),
    _degreeToMinByteLength(getHighestDegree(m) - 1),
  );
...
export class RabinPolynomialEncoder {
  ...
  getFingerprint() {
    return toUint8Array(
      this.fingerprint,
      _degreeToMinByteLength(getHighestDegree(this.m) - 1),
    );
  }
}
