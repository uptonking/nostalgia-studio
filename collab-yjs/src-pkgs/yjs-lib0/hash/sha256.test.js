...
export const testSha256Basics = async (_tc) => {
  ...
  const test = async (data, result) => {
    ...
    const resWebcrypto = new Uint8Array(
  ...
  await test(
    string.encodeUtf8('ab'),
    'fb8e20fc2e4c3f248c60c39bd652f3c1347298bb977b8b4d5903b85055620603',
  );
  await test(
    string.encodeUtf8('abc'),
    'ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad',
  );
  await test(
    string.encodeUtf8('abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopno'),
  ...
  await test(
    ...
    'e54ad86cad2d9d7c03506d6a67ed03fab5fbb8d34012143e9c0eb88ace56ca59',
  );
  await test(
    string.encodeUtf8(
      'abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopqqqq00012',
    ),
    '7a1e062405a534817dcb89fa2416a69e4fbe75fabece33d528b82d1b71d3e418',
  );
  await test(
    string.encodeUtf8(
...
export const testLargeValue = async (_tc) => {
  ...
  let resNode = buffer.fromBase64(
    '81m7UtkH2s9J3E33Bw5kRMuC5zvktgZ64SfzenbV5Lw=',
  );
  let resLib0;
  let resWebcrypto;
  if (env.isNode) {
    ...
    t.measureTime(`[node] Hash message of size ${BS}`, () => {
      ...
      if (!f.equalityDeep(res, resNode)) {
        console.warn(
          `Precomputed result should be the same! New result: ${buffer.toBase64(res)}`,
        );
      }
      resNode = res;
  ...
  await t.measureTimeAsync(
    `[webcrypto] Hash message of size ${BS}`,
    async () => {
      resWebcrypto = new Uint8Array(
        await webcrypto.subtle.digest('SHA-256', data),
      );
    },
  );
...
export const testRepeatSha256Hashing = async (tc) => {
  const LEN = prng.bool(tc.prng)
    ? prng.uint32(tc.prng, 0, 512)
    : prng.uint32(tc.prng, 0, 3003030);
  const data = prng.uint8Array(tc.prng, LEN);
  const hashedCustom = sha256.digest(data);
  const hashedWebcrypto = new Uint8Array(
    await webcrypto.subtle.digest('SHA-256', data),
  );
