      ...
      if (templateElement) {
        ...
        for (const key in childStates) {
          this._childStates.push({
            d: /** @type {Lib0Component<T>} */ (
              dom.querySelector(/** @type {any} */ (shadow), key)
            ),
            s: childStates[key],
          });
        }
        ...
        if (state) {
          ...
          for (const key in slotElems) {
            ...
            if (this._slots[key] !== slotContent) {
              ...
              const currentSlots = /** @type {Array<any>} */ (
                key !== 'default'
  ...
  return createComponent('lib0-list', {
    ...
    onStateChange: (state, prevState, component) => {
      ...
      component.insertBefore(
        dom.fragment(
          insert.slice(insertStart).map(
            /** @param {any} insState */ (insState) => {
              const el = new Item();
