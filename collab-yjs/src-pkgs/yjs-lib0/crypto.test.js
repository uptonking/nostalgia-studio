...
export const testJwt = async (_tc) => {
  const publicJwk = json.parse(
    '{"key_ops":["verify"],"ext":true,"kty":"EC","x":"X7xPIgxWOHmKPv2PtrxGvaQUJ3LiUXQTVLExwPGBvanD3kAc9sEY9FwKxp8NVJ3j","y":"SIBaHLE1fvW_O-xOdzmbkU5M_M7cGHULZHrXOo_exCKBIbV2pJm3MH87gAXkZvoD","crv":"P-384"}',
  );
  const privateJwk = json.parse(
    '{"key_ops":["sign"],"ext":true,"kty":"EC","x":"X7xPIgxWOHmKPv2PtrxGvaQUJ3LiUXQTVLExwPGBvanD3kAc9sEY9FwKxp8NVJ3j","y":"SIBaHLE1fvW_O-xOdzmbkU5M_M7cGHULZHrXOo_exCKBIbV2pJm3MH87gAXkZvoD","crv":"P-384","d":"3BdPp9LSWOl36bJuwEIun14Y17dgV7AK8RKqOuTJAbG080kemtr7qmZgTiCE_K_o"}',
  );
...
export const testImportExport = async (tc) => {
  ...
  await t.groupAsync('ecdsa (jwk))', async () => {
    ...
    const ekeyPrivate = await ecdsa.importKeyJwk(jwkPrivate, {
      extractable: true,
    });
    const ekeyPublic = await ecdsa.importKeyJwk(jwkPublic, {
      extractable: true,
    });
  ...
  await t.groupAsync('ecdsa (raw))', async () => {
    ...
    const ekeyPublic = await ecdsa.importKeyRaw(rawPublic, {
      extractable: true,
      usages: ['verify'],
    });
  ...
  await t.groupAsync('rsa-oaep (jwk))', async () => {
    ...
    const ekeyPrivate = await rsa.importKeyJwk(jwkPrivate, {
      extractable: true,
    });
...
export const testEncryptionPerformance = async (tc) => {
  ...
  await t.measureTimeAsync(
    `Encrypt ${N / 1000}k blocks of size ${BLen}byte`,
    async () => {
  ...
  const decryptedData = [];
  await t.measureTimeAsync(
    `Decrypt ${N / 1000}k blocks of size ${BLen}byte`,
    async () => {
...
export const testConsistentKeyGeneration = async (_tc) => {
  ...
  await t.groupAsync('key generation (ECDSA))', async () => {
    ...
    const privateKey = await ecdsa.importKeyJwk(jwkPrivate, {
      extractable: true,
      usages: ['sign'],
    });
    const publicKey = await ecdsa.importKeyJwk(jwkPublic, {
      extractable: true,
      usages: ['verify'],
    });
  ...
  await t.groupAsync('key generation (RSA))', async () => {
    ...
    const privateKey = await rsa.importKeyJwk(jwkPrivate, {
      extractable: true,
      usages: ['decrypt'],
    });
    const publicKey = await rsa.importKeyJwk(jwkPublic, {
      extractable: true,
      usages: ['encrypt'],
    });
...
export const testSigning = async (tc) => {
  await t.measureTimeAsync(
    'time to sign & verify 2 messages (ECDSA))',
    async () => {
      const keypair = await ecdsa.generateKeyPair({ extractable: true });
