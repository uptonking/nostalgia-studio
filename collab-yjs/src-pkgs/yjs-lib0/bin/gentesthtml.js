...
const extractModMap = (v, k, pkgName, pathPrefix, importMap) => {
  ...
  if (typeof v === 'object') {
    extractModMap(
      v.browser || v.module || v.default || v.import,
      k,
      pkgName,
      pathPrefix,
      importMap,
...
const readPkg = (pkgJson, pathPrefix, importMap) => {
  object.forEach(pkgJson.exports, (v, k) =>
    extractModMap(v, k, pkgJson.name, pathPrefix, importMap),
  );
  object.forEach(pkgJson.dependencies, (_v, depName) => {
    const nextImportMap =
      pathPrefix === '.' ? exports : (scopes[pathPrefix + '/'] = {});
    const prefix = `./node_modules/${depName}`;
    const depPkgJson = JSON.parse(
      fs.readFileSync(prefix + '/package.json', { encoding: 'utf8' }),
    );
