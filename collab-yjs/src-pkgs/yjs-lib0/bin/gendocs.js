    ...
    /** @param {Array<any>} json */ (json) => {
      ...
      json.forEach((item) => {
        if (item.meta && item.meta.filename) {
          const mod =
            modules[item.meta.filename] ||
            (modules[item.meta.filename] = {
              items: [],
              name: item.meta.filename.slice(0, -3),
              description: '',
            });
          if (item.kind === 'module') {
            mod.name = item.name;
      ...
      for (const fileName in modules) {
        ...
        strBuilder.push(
          `<details><summary><b>[lib0/${mod.name}]</b> ${descHead}</summary>`,
        );
        strBuilder.push(
          `<pre>import * as ${mod.name} from 'lib0/${fileName.slice(0, -3)}'</pre>`,
        );
            ...
            switch (item.kind) {
              ...
              case 'constant': {
                if (item.params == null && item.returns == null) {
                  ...
                  strBuilder.push(
                    `<b><code>${item.longname.slice(7)}${typeEval ? ': ' + toSafeHtml(typeEval[1]) : ''}</code></b><br>`,
                  );
                  if (item.description) {
              ...
              case 'function': {
                ...
                const paramVal = (item.params || [])
                  .map(
                    /** @param {any} ret */ (ret) =>
                      `${ret.name}: ${getOriginalParamTypeDecl(ret.name) || ret.type.names.join('|')}`,
                  )
                  .join(', ');
                ...
                const returnVal = evalReturnRegex
                  ...
                  : item.returns
                    ? item.returns
                        .map(
                          /** @param {any} r */ (r) => r.type.names.join('|'),
                        )
                        .join('|')
                    : '';
                strBuilder.push(
                  `<b><code>${item.kind === 'class' ? 'new ' : ''}${item.longname.slice(7)}(${toSafeHtml(paramVal)})${toSafeHtml(returnVal)}</code></b><br>`,
                );
                const desc =
                  item.description ||
                  item.classdesc ||
                  classDescriptions[item.longname] ||
                  null;
                if (desc) {
              ...
              case 'member': {
                if (item.type) {
                  strBuilder.push(
                    `<b><code>${item.longname.slice(7)}: ${toSafeHtml(/** @type {RegExpExecArray} */ (jsdocTypeRegex.exec(item.comment))[1])}</code></b><br>`,
                  );
                  if (item.description) {
                    strBuilder.push(`<dd>${item.description}</dd>`);
                  }
