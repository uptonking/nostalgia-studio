...
export const testStringEncodingPerformanceNativeVsPolyfill = () => {
  ...
  t.assert(
    nativeTime < customTime,
    'We expect native encoding to be more performant for large data sets',
  );

  const smallRepetitions = 100000;
  const customTimeSmall = t.measureTime(
    'small dataset: custom encoding',
    () => {
      const encoder = encoding.createEncoder();
  ...
  const nativeTimeSmall = t.measureTime(
    'small dataset: native encoding',
    () => {
      const encoder = encoding.createEncoder();
      for (let i = 0; i < smallRepetitions; i++) {
        encoding._writeVarStringNative(encoder, 'i');
  ...
  t.assert(
    nativeTimeSmall < customTimeSmall * 5,
    'We expect native encoding to be not much worse than custom encoding for small data sets',
  );
};
...
export const testDecodingPerformanceNativeVsPolyfill = () => {
  ...
  const polyfillTimeSmall = t.measureTime(
    'small dataset: polyfill encoding',
    () => {
      const decoder = decoding.createDecoder(buf);
      while (decoding.hasContent(decoder)) {
  ...
  const nativeTimeLarge = t.measureTime(
    'large dataset: native encoding',
    () => {
  ...
  const polyfillTimeLarge = t.measureTime(
    'large dataset: polyfill encoding',
    () => {
      const decoder = decoding.createDecoder(bufLarge);
      while (decoding.hasContent(decoder)) {
  ...
  t.assert(
    nativeTimeSmall < polyfillTimeSmall * 2.0,
    'Small dataset: We expect native decoding to be not much worse than',
  );
  t.assert(
    nativeTimeLarge < polyfillTimeLarge * 1.5,
    'Large dataset: We expect native decoding to be much better than polyfill decoding',
  );
};
...
export const testStringDecodingPerformance = () => {
  ...
  t.assert(
    durationConcatElements < 2 * durationSingleElements,
...
const encodingPairs = [
  ...
  },
  {
    ...
    gen: (gen) =>
      prng.int53(gen, number.MIN_SAFE_INTEGER, number.MAX_SAFE_INTEGER),
    compare: strictComparison,
  },
  {
    name: 'Any',
  ...
  const intEncoders = [
    {
      ...
      read: (encoder) =>
        new decoding.IntDiffOptRleDecoder(encoder.toUint8Array()),
    },
    {
...
export const testIntDiffEncoder = (_tc) => {
  ...
  const decoder = new decoding.IntDiffDecoder(
    encoding.toUint8Array(encoder),
    0,
  );
  for (let i = -N; i < N; i++) {
...
export const testLargeNumberEncoding = (tc) => {
  const encoder = encoding.createEncoder();
  const num1 = -2.2062063918362897e50;
  const num2 = BigInt(
    prng.int53(tc.prng, number.MIN_SAFE_INTEGER, number.MAX_SAFE_INTEGER),
  );
