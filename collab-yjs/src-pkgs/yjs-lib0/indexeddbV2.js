...
export const deleteDB = (name) =>
  pledge.createWithDependencies(
    (p, name) => bindPledge(p, indexedDB.deleteDatabase(name)),
    name,
  );
...
export const count = (store, range) =>
  pledge.createWithDependencies(
    (p, store, range) => bindPledge(p, store.count(range)),
    store,
    range,
  );
...
export const get = (store, key) =>
  pledge.createWithDependencies(
    (p, store, key) => bindPledge(p, store.get(key)),
    store,
    key,
  );
...
export const del = (store, key) =>
  pledge.createWithDependencies(
    (p, store, key) => bindPledge(p, store.delete(key)),
    store,
    key,
  );
...
export const put = (store, item, key) =>
  pledge.createWithDependencies(
    (p, store, item, key) => bindPledge(p, store.put(item, key)),
    store,
    item,
    key,
  );
...
export const add = (store, item, key) =>
  pledge.createWithDependencies(
    (p, store, item, key) => bindPledge(p, store.add(item, key)),
    store,
    item,
    key,
  );
...
export const addAutoKey = (store, item) =>
  pledge.createWithDependencies(
    (p, store, item) => bindPledge(p, store.add(item)),
    store,
    item,
  );
...
export const getAll = (store, range, limit) =>
  pledge.createWithDependencies(
    (p, store, range, limit) => bindPledge(p, store.getAll(range, limit)),
    store,
    range,
    limit,
  );
...
export const getAllKeys = (store, range, limit) =>
  pledge.createWithDependencies(
    (p, store, range, limit) => bindPledge(p, store.getAllKeys(range, limit)),
    store,
    range,
    limit,
  );
 ...
 * @return {pledge.PledgeInstance<any>}
 */
export const getLastKey = (store, range = null) =>
  queryFirst(store, range, 'prev');
 ...
 * @return {pledge.PledgeInstance<any>}
 */
export const getFirstKey = (store, range = null) =>
  queryFirst(store, range, 'next');
...
export const getAllKeysValues = (store, range, limit) =>
  pledge.createWithDependencies(
    (p, store, range, limit) => {
      pledge
        .all([getAllKeys(store, range, limit), getAll(store, range, limit)])
        .map(([ks, vs]) => ks.map((k, i) => ({ k, v: vs[i] })))
        .whenResolved(p.resolve.bind(p));
    },
    store,
    range,
    limit,
...
export const iterate = (store, keyrange, f, direction = 'next') =>
  pledge.createWithDependencies(
    (p, store, keyrange) => {
      iterateOnRequest(p, store.openCursor(keyrange, direction), (cursor) =>
...
export const iterateKeys = (store, keyrange, f, direction = 'next') =>
  pledge.createWithDependencies(
    (p, store, keyrange) => {
      iterateOnRequest(p, store.openKeyCursor(keyrange, direction), (cursor) =>
        f(cursor.key),
      );
    },
    store,
