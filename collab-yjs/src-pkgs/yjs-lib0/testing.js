...
/* c8 ignore next */
export const envSeed = env.hasParam('--seed')
  ? Number.parseInt(env.getParam('--seed', '0'))
  : null;
...
/* c8 ignore next */
const testFilter = env.hasParam('--filter')
  ? env.getParam('--filter', '')
  : null;
...
export const run = async (moduleName, name, f, i, numberOfTests) => {
  ...
  const filtered = !testFilterRegExp.test(
    `[${i + 1}/${numberOfTests}] ${moduleName}: ${uncamelized}`,
  );
  /* c8 ignore next 3 */
  if (filtered) {
    return true;
 ...
 * @param {string} info
 */
export const describe = (description, info = '') =>
  log.print(log.BLUE, description, ' ', log.GREY, info);
...
export const compareStrings = (a, b, m = 'Strings match') => {
  if (a !== b) {
    ...
    log.print(
      log.GREY,
      a.slice(0, diff.index),
      log.RED,
      a.slice(diff.index, diff.remove),
      log.GREEN,
      diff.insert,
      log.GREY,
      a.slice(diff.index + diff.remove),
 ...
 */
export const compareObjects = (a, b, m = 'Objects match') => {
  object.equalFlat(a, b) || fail(m);
};
...
const compareValues = (_constructor, a, b, path) => {
  if (a !== b) {
    fail(
      `Values ${json.stringify(a)} and ${json.stringify(b)} don't match (${path})`,
    );
  }
  return true;
};
 ...
 * @throws {TestError}
 */
const _failMessage = (message, reason, path) =>
  fail(
  ...
  switch (a.constructor) {
    ...
    case Object:
      if (object.length(a) !== object.length(b)) {
        _failMessage(
          message,
          'Objects have a different number of attributes',
          path,
        );
      }
      object.forEach(a, (value, key) => {
        if (!object.hasProperty(b, key)) {
          _failMessage(
            message,
            `Property ${path} does not exist on second argument`,
            path,
          );
        }
    ...
    case Array:
      if (a.length !== b.length) {
        _failMessage(
          message,
          'Arrays have a different number of attributes',
          path,
        );
      }
      // @ts-ignore
    ...
    default:
      if (!customCompare(a.constructor, a, b, path, compareValues)) {
        _failMessage(
          message,
          `Values ${json.stringify(a)} and ${json.stringify(b)} don't match`,
          path,
        );
      }
  }
 ...
 */
export const compare = (a, b, message = null, customCompare = compareValues) =>
  _compare(a, b, 'obj', message, customCompare);
 ...
 */
/* c8 ignore next */
export const assert = (property, message = null) => {
  property || fail(`Assertion failed${message !== null ? `: ${message}` : ''}`);
};
...
export const runTests = async (tests) => {
   ...
   * @param {string} testname
   */
  const filterTest = (testname) =>
    testname.startsWith('test') || testname.startsWith('benchmark');
  const numberOfTests = object
    .map(tests, (mod) =>
      object
        .map(mod, (f, fname) =>
          /* c8 ignore next */ f && filterTest(fname) ? 1 : 0,
        )
        .reduce(math.add, 0),
    )
    .reduce(math.add, 0);
  ...
  if (success) {
    log.print(
      ...
      log.GREY,
      log.UNBOLD,
      ` in ${time.humanizeDuration(end - start)}`,
    );
