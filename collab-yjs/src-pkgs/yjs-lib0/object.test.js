...
export const testObject = (_tc) => {
  t.assert(
    object.create().constructor === undefined,
    'object.create creates an empty object without constructor',
  );
  ...
  t.assert(
    object.equalFlat({ x: 'dtrn' }, { x: 'dtrn' }),
    'comparing equal objects',
  );
  t.assert(
    !object.equalFlat({ x: {} }, { x: {} }),
    'flatEqual does not dive deep',
  );
  t.assert(
    object.equalFlat({ x: undefined }, { x: undefined }),
    'flatEqual handles undefined',
  );
  t.assert(
    !object.equalFlat({ x: undefined }, { y: {} }),
    'flatEqual handles undefined',
  );
  t.describe('object.every');
  t.assert(object.every({ a: 1, b: 3 }, (v, k) => v % 2 === 1 && k !== 'c'));
  t.assert(
    !object.every({ a: 1, b: 3, c: 5 }, (v, k) => v % 2 === 1 && k !== 'c'),
  );
