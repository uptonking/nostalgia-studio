...
export const nottestingNotTested = () => {
  t.assert(
    false,
    'This test should not be executed because the name doesnt start with "test"',
  );
};
...
export const testComparing = (_tc) => {
  ...
  t.fails(() => {
    t.compare(
      { a: 4 },
      { a: 4, b: 5 },
      'childs have different length (object)',
    );
  });
  t.fails(() => {
...
export const testFailing = async () => {
  ...
  await t.promiseRejected(() =>
    promise.reject(error.create('should be rejected')),
  );
  t.fails(() => {
    t.fails(() => {});
  });
  ...
  await t.promiseRejected(() => t.promiseRejected(() => promise.resolve()));
};
