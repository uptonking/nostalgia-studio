...
const _stringify = (data) =>
  buffer.toBase64UrlEncoded(string.encodeUtf8(json.stringify(data)));

/**
 * @param {string} base64url
 */
const _parse = (base64url) =>
  json.parse(string.decodeUtf8(buffer.fromBase64UrlEncoded(base64url)));
...
export const encodeJwt = (privateKey, payload) => {
  const { name: algName, namedCurve: algCurve } = /** @type {any} */ (
  ...
  return ecdsa
    .sign(privateKey, string.encodeUtf8(jwt))
    .then((signature) => jwt + '.' + buffer.toBase64UrlEncoded(signature));
};
...
export const verifyJwt = async (publicKey, jwt) => {
  ...
  const verified = await ecdsa.verify(
    publicKey,
    buffer.fromBase64UrlEncoded(signatureBase64),
    string.encodeUtf8(headerBase64 + '.' + payloadBase64),
  );
  /* c8 ignore next 3 */
  if (!verified) {
