...
import { _omit, convertToHex } from './common';
...
export function matchTableCell(node, delta, scroll) {
  ...
  const cellBg =
    node.getAttribute('data-cell-bg') || node.style.backgroundColor; // The td from external table has no 'data-cell-bg'
  ...
  return delta.reduce((newDelta, op) => {
    if (
      ...
      if (
        ...
        op.attributes.background === convertToHex(cellBg)
      ) {
        newDelta.insert(
          op.insert,
          Object.assign(
        ...
        newDelta.insert(
          op.insert,
          Object.assign({}, _omit(op.attributes, ['table', 'table-cell-line'])),
        );
      }
    }
...
export function matchTable(node, delta, scroll) {
  ...
  const cellsInTopRow = Array.from(topRow.querySelectorAll('td')).concat(
    Array.from(topRow.querySelectorAll('th')),
  );
  const maxCellsNumber = cellsInTopRow.reduce(
    // @ts-expect-error fix-types
    (sum: number, cell: HTMLElement) => {
