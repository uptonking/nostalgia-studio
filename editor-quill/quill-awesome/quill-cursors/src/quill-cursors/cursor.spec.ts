...
describe('Cursor', () => {
  ...
  it('updates the caret position', () => {
    ...
    const caretContainer = element.getElementsByClassName(
  ...
  it('toggles the flag display', () => {
    const cursor = new Cursor('abc', 'Jane Bloggs', 'red');
    const element = cursor.build(options);
    const flag = element.getElementsByClassName(
      Cursor.CARET_CONTAINER_CLASS,
    )[0];
  ...
  describe('with some selections', () => {
    ...
    it('adds selections', () => {
      const selections = element.getElementsByClassName(
      ...
      expect(selections.children[0]).toHaveStyle(
      ...
      expect(selections.children[1]).toHaveStyle(
        'background-color: rgba(255, 0, 0, 0.3)',
      );
    });
