...
import type { ICoordinates } from './i-coordinates';
...
export default class Cursor {
  ...
  public build(options: IQuillCursorsOptions): HTMLElement {
    ...
    element.getElementsByClassName(Cursor.NAME_CLASS)[0].textContent =
      this.name;
  ...
  public toggleFlag(shouldShow?: boolean): void {
    const isShown = this._caretEl.classList.toggle(
      Cursor.CONTAINER_HOVER_CLASS,
      shouldShow,
    );
    if (isShown) return;
    ...
    setTimeout(
      () => this._flagEl.classList.remove(Cursor.NO_DELAY_CLASS),
      this._hideSpeedMs,
    );
  }
  ...
  public updateSelection(
    ...
    selections.forEach((selection: ClientRect) =>
  ...
  private _toggleOpenedCursor(e: MouseEvent): void {
    ...
    this._caretEl.classList.toggle(
      Cursor.CONTAINER_NO_POINTER_CLASS,
      shouldShow,
    );
    if (!shouldShow)
      document.removeEventListener('mousemove', this._toggleOpenedCursor);
  }
