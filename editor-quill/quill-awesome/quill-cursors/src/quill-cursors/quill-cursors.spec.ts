...
describe('QuillCursors', () => {
  ...
  describe('initialisation', () => {
    ...
    it('registers a scroll listener', () => {
      ...
      expect(editor.addEventListener).toHaveBeenCalledWith(
        'scroll',
        expect.anything(),
      );
  ...
  describe('text change listener', () => {
    ...
    beforeEach(() => {
      ...
      jest.spyOn(quill, 'on').mockImplementation(((
        event: string,
        callback: Function,
      ) => {
    ...
    it('emits the selection on text change', () => {
      ...
      jest
        .spyOn(quill, 'getSelection')
        .mockReturnValue({ index: 10, length: 10 });
      listeners['text-change']();
      jest.runAllTimers();
    ...
    it('transforms an existing cursor after an insertion', () => {
      ...
      const delta = [{ retain: 5 }, { insert: 'foo' }];
  ...
  describe('tracking current selection', () => {
    ...
    beforeEach(() => {
      ...
      jest.spyOn(quill.emitter, 'emit').mockImplementation(((
        event: string,
        ...args: any[]
      ) => {
    ...
    it('updates the current selection on text change', () => {
      ...
      jest
        .spyOn(quill, 'getSelection')
        .mockReturnValue({ index: 20, length: 20 });
      quill.emitter.emit('text-change');
      jest.runAllTimers();
  ...
  describe('creating a cursor', () => {
    ...
    it('adds the cursor to the DOM', () => {
      ...
      const cursorsContainer =
        quill.container.getElementsByClassName('ql-cursors')[0];
  ...
  describe('updating cursors', () => {
    ...
    it('selects a block embed element', () => {
      ...
      jest.spyOn(quill, 'getLeaf').mockReturnValue(createLeaf());
      jest.spyOn(quill, 'getLines').mockReturnValue([{ domNode: img }]);
  ...
  describe('flag', () => {
    it('toggles the flag of a cursor', () => {
      ...
      const container = quill.container.getElementsByClassName(
