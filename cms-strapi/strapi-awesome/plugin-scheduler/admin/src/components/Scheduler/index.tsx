...
import {
  IScheduler,
  ISchedulerControllerReturn,
} from '../../../../server/controllers/scheduler';
import {
  IConfig,
  IConfigControllerReturn,
} from '../../../../server/controllers/config';
import DateTimePickerWrapper from '../date-time-picker-wrapper';

const DATETIME_PICKER_STEP_SIZE = 5;

const Scheduler = () => {
  const { onChange, layout, initialData, modifiedData, isCreatingEntry } =
    useCMEditViewDataManager();
  ...
  const [scheduler, setScheduler] = useState<IScheduler | undefined | null>(
    undefined,
  );
  ...
  const fetchConfig = async (uid: string) => {
    try {
      const configFetchResult: { data?: IConfigControllerReturn } = await get(
        `/scheduler/config/${uid}`,
      );
  ...
  const fetchScheduler = async (uid: string, entryId: string) => {
    try {
      const schedulerFetchResult: { data: ISchedulerControllerReturn } =
        await get(`/scheduler/scheduler/${uid}/${entryId}`);
  ...
  useEffect(() => {
    if (isLoadingConfig === false && isLoadingScheduler === false) {
      ...
      if (initialPublishAt !== undefined) {
        const value =
          initialPublishAt === null ? null : new Date(initialPublishAt);
        updateFormValue('publishAt', value, true);
      }

      if (initialArchiveAt !== undefined) {
        const value =
          initialArchiveAt === null ? null : new Date(initialArchiveAt);
...
const getInitialPublishAt = (
  ...
  if (
    config?.initialPublishAtDate === null ||
    config?.initialPublishAtDate !== undefined
  ) {
...
const getInitialArchiveAt = (
  ...
  if (
    config?.initialArchiveAtDate === null ||
    config?.initialArchiveAtDate !== undefined
  ) {
