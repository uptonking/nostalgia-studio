...
export const TagsToolbarButton = ({ disabled }: TagsToolbarButtonProps) => {
  ...
  const handleNewTagFormSubmit = async (
  ...
  const handleTagRename: TagAccordionProps['onTagRename'] = async (
    uuid,
    { name },
  ) => {
  ...
  const handleApplyTagsSubmit: React.FormEventHandler<HTMLFormElement> = async (
    e,
  ) => {
    e.preventDefault();
    ...
    const connectedTagUUIDs = Array.from(data.getAll('tags')).map((uuid) =>
      uuid.toString(),
    );
            ...
            <>
              ...
              <Dialog.Content forceMount asChild>
                <DialogContent style={{ x }}>
                  ...
                  <VisuallyHidden>
                    <Dialog.Description>
                      {'Create, delete & manage your tags.'}
                    </Dialog.Description>
                  </VisuallyHidden>
                  {hasSelectedItems ? (
                    <TagContainer as='form' onSubmit={handleApplyTagsSubmit}>
                      ...
                      <TextButton aria-busy={isUpdateFileLoading}>
                        Apply tags
                      </TextButton>
                    </TagContainer>
                  ) : (
...
const TagItem = styled.li<{ $isForm?: boolean; $disabled?: boolean }>`
  background-color: ${(props) =>
    props.$isForm ? 'transparent' : 'rgba(255, 255, 255, 0.04)'};
  border-radius: 5px;
...
interface NewTagFormProps
  extends Pick<TextInputProps, 'onBlur' | 'initialValue'> {
...
const CheckboxInner = styled.span<{ $variant: CheckboxState }>`
  background: ${(props) =>
    props.$variant !== 'unchecked' ? '#fafafa' : 'transparent'};
  border-radius: 1px;
  padding: 4px;
  clip-path: ${(props) =>
