...
class FilesService implements IFilesService {
  ...
  private uploadFileAndPersist = async (
    ...
    if (file.assetType === 'image') {
      const imageWithDimensions =
        await getService<IImagesService>('images').upload(file);
      ...
      const uploadedFile =
        await getService<IProviderService>('provider').upload(file);
  ...
  deleteFile = async (uuid: string): Promise<FileEntity | undefined> => {
    ...
    const deletedFile = await this.strapi.entityService.delete(
      FILE_MODEL_UID,
      fileID,
    );
