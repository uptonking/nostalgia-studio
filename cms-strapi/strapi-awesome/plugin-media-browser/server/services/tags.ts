...
interface ExtendedFindParams<TEntity>
  extends Omit<FindParams<TEntity>, 'populate'> {
  populate?:
    | (keyof TEntity)[]
    | { [Key in keyof TEntity]?: { count: boolean } | boolean };
}
...
export interface ITagsService {
  ...
  delete(
    uuids: string[],
  ): Promise<{ totalTagNumber: number; tags: UpdatedTag[] }>;
...
class TagsService implements ITagsService {
  ...
  findAll = async (
    query: ExtendedFindParams<TagEntity>,
  ...
  delete = async (
    ...
    const tags = (await this.strapi.db.query(TAG_MODEL_UID).findMany({
      select: ['name', 'uuid'],
      where: { uuid: { $in: uuids } },
    })) as UpdatedTag[];
    ...
    const { count: totalTagNumber } = await this.strapi.db
      .query(TAG_MODEL_UID)
      .deleteMany({
        where: {
  ...
  update = async (
    uuid: string,
    patch: TagEntityPatch,
  ): Promise<UpdatedTag | undefined> => {
