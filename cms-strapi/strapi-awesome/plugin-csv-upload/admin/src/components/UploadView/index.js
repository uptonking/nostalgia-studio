...
function statusToMessage(status) {
  ...
  return (
    <span style={{ color: status >= 200 && status < 400 ? '#0a0' : '#e00' }}>
      {message}
    </span>
  );
}
...
export default function (props) {
  ...
  const successCount = progresses.filter(
    (progress) => progress >= 200 && progress < 300,
  ).length;
  ...
  return (
    <>
      <Button
        ...
        disabled={
          csv.data.length === 0 || currentProgress === ProgressType.Saving
        }
        loading={currentProgress === ProgressType.Saving}
        onClick={onClickSave}
      >
        {formatMessage({
          id: 'csv-upload.ContentType.Save',
          defaultMessage: 'Save',
        })}
      </Button>
        ...
        <Table rowCount={1} colCount={attributes.length + 2}>
          ...
          <Tbody>
            {csv.data.map((line, lineIndex) => (
              <Tr key={lineIndex}>
                <Td>{statusToMessage(progresses[lineIndex])}</Td>
                {Object.values(Object.assign({}, initialValues, line)).map(
                  (cell, cellIndex) => {
                    const attribute =
                      attributes[cellIndex] && attributes[cellIndex].length > 1
                        ? attributes[cellIndex][1]
                        : {};
                    return (
                      <EditableCell
                        key={cellIndex}
                      ...
                      >
                        <AttributeCell value={cell} attribute={attribute}>
                          {cell}
                        </AttributeCell>
                      </EditableCell>
                    );
                ...
                <Td>
                  {currentProgress === ProgressType.Prepare && (
                    <IconButton
                      noBorder={true}
                      icon={<Trash />}
                      onClick={() => {
                        const newCSVData = csv.data.filter(
                          (row, index) => index !== lineIndex,
                        );
