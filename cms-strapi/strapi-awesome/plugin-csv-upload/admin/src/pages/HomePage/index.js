...
const HomePage = () => {
  ...
  const onSetCSVData = useCallback(
    (data) => {
  ...
  useEffect(() => {
    fetchContentTypeRequest().then((response) => {
      dispatch(
        onChangeContentTypes(
          response.data.data.filter((row) => row.schema.visible),
        ),
      );
    });
  }, []);
  ...
  const onChangeFile = async (e) => {
    ...
    const selectContentType = contentTypes.data.find(
      (value) => value.uid === contentTypes.active,
    );
    ...
    try {
      ...
      for (const attribute in selectContentType.schema.attributes) {
        attributeTypes[attribute] =
          selectContentType.schema.attributes[attribute];
      }
  ...
  return (
    <>
      <BaseHeaderLayout title='CSV Upload' as='h2' />
      <Box paddingLeft={10} paddingRight={10} paddingTop={2}>
        <NavigationBar>
          ...
          >
            {formatMessage({
              id: 'csv-upload.Button.CSVUpload',
        ...
        {contentTypes.active && csv.data.length > 0 && (
          <UploadView
            contentType={contentTypes.data.find(
              (value) => value.uid === contentTypes.active,
            )}
        ...
        {contentTypes.active &&
          csv.data.length === 0 &&
          (() => {
            const activeContentType = contentTypes.data.find(
              (contentType) => contentType.uid === contentTypes.active,
            );
            const headers = activeContentType
              ? Object.keys(activeContentType.schema.attributes)
              : [];
