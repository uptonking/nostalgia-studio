...
function DataMap(instance, priv, GridSettings) {
  ...
  this.instance.addHook('skipLengthCache', (delay) =>
    this.onSkipLengthCache(delay),
  );
...
DataMap.prototype.createMap = function () {
  ...
  if (typeof schema === 'undefined') {
    throw new Error(
      "trying to create `columns` definition but you didn't provide `schema` nor `data`",
    );
  }
...
DataMap.prototype.colToProp = function (col) {
  ...
  if (
    !isNaN(physicalColumn) &&
    this.colToPropCache &&
    typeof this.colToPropCache[physicalColumn] !== 'undefined'
  ) {
...
DataMap.prototype.createRow = function (index, amount = 1, source) {
  ...
  while (
    ...
    this.instance.countSourceRows() < maxRows
  ) {
    let row = null;
...
DataMap.prototype.createCol = function (index, amount = 1, source) {
  if (!this.instance.isColumnModificationAllowed()) {
    throw new Error(
      'Cannot create new column. When data source in an object, ' +
  ...
  const columnIndex =
    typeof index !== 'number' || index >= countColumns ? countColumns : index;
  let constructor;
  ...
  while (numberOfCreatedCols < amount && this.instance.countCols() < maxCols) {
    constructor = columnFactory(
      this.GridSettings,
      this.priv.columnsSettingConflicts,
    );
...
DataMap.prototype.removeRow = function (index, amount = 1, source) {
  ...
  const rowsAmount = this.instance.runHooks(
    'modifyRemovedAmount',
    amount,
    rowIndex,
  );
  ...
  rowIndex =
    (this.instance.countSourceRows() + rowIndex) %
...
DataMap.prototype.removeCol = function (index, amount = 1, source) {
  if (
    this.instance.dataType === 'object' ||
    this.instance.getSettings().columns
  ) {
    throw new Error(
  ...
  columnIndex =
    (this.instance.countCols() + columnIndex) % this.instance.countCols();
...
DataMap.prototype.filterData = function (index, amount) {
  ...
  if (continueSplicing !== false) {
    const newData = this.dataSource.filter(
      (row, rowIndex) => physicalRows.indexOf(rowIndex) === -1,
    );
...
DataMap.prototype.set = function (row, prop, value, source) {
  const physicalRow = this.instance.runHooks(
    'modifyRow',
    row,
    source || 'datamapGet',
  );
...
DataMap.prototype.getRange = function (start, end, destination) {
  ...
  const getFn =
    destination === this.DESTINATION_CLIPBOARD_GENERATOR
      ? this.getCopyable
      : this.get;
...
DataMap.prototype.getText = function (start, end) {
  return SheetClip.stringify(
    this.getRange(start, end, this.DESTINATION_RENDERER),
  );
};
...
DataMap.prototype.getCopyableText = function (start, end) {
  return SheetClip.stringify(
    this.getRange(start, end, this.DESTINATION_CLIPBOARD_GENERATOR),
  );
};
