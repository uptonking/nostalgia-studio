...
class Viewport {
  ...
  getWorkspaceHeight() {
    const trimmingContainer =
      this.instance.wtOverlays.topOverlay.trimmingContainer;
    let elemHeight;
    let height = 0;
    ...
    if (trimmingContainer === window) {
      ...
      height =
        elemHeight > 0 && trimmingContainer.clientHeight > 0
          ? trimmingContainer.clientHeight
          : Infinity;
    }
  ...
  getWorkspaceWidth() {
    ...
    const trimmingContainer =
      this.instance.wtOverlays.leftOverlay.trimmingContainer;
    let overflow;
    ...
    if (this.wot.getSetting('freezeOverlays')) {
      width = Math.min(
        docOffsetWidth - this.getWorkspaceOffset().left,
        docOffsetWidth,
      );
    } else {
      width = Math.min(
        this.getContainerFillWidth(),
        docOffsetWidth - this.getWorkspaceOffset().left,
        docOffsetWidth,
      );
    }
  ...
  getWorkspaceActualWidth() {
    return (
      outerWidth(this.wot.wtTable.TABLE) ||
      outerWidth(this.wot.wtTable.TBODY) ||
      outerWidth(this.wot.wtTable.THEAD)
  ...
  getRowHeaderWidth() {
    ...
    if (rowHeadersHeightSetting) {
      ...
      for (let i = 0, len = rowHeaders.length; i < len; i++) {
        this.rowHeaderWidth +=
          rowHeadersHeightSetting[i] || rowHeadersHeightSetting;
      }
    }
    ...
    this.rowHeaderWidth =
      this.instance.getSetting('onModifyRowHeaderWidth', this.rowHeaderWidth) ||
      this.rowHeaderWidth;
  ...
  createRowsCalculator(visible = false) {
    ...
    let pos =
      ...
      this.wot.wtOverlays.topOverlay.getTableParentOffset();
    ...
    if (fixedRowsTop) {
      fixedRowsHeight = this.wot.wtOverlays.topOverlay.sumCellSizes(
        0,
        fixedRowsTop,
      );
      pos += fixedRowsHeight;
    ...
    if (fixedRowsBottom && this.wot.wtOverlays.bottomOverlay.clone) {
      fixedRowsHeight = this.wot.wtOverlays.bottomOverlay.sumCellSizes(
    ...
    if (
      this.wot.wtTable.holder.clientHeight ===
      this.wot.wtTable.holder.offsetHeight
    ) {
      scrollbarHeight = 0;
    } else {
    ...
    return new ViewportRowsCalculator(
      ...
      visible
        ? null
        : this.wot.wtSettings.settings.viewportRowCalculatorOverride,
      visible,
      scrollbarHeight,
    );
  }
  ...
  createColumnsCalculator(visible = false) {
    ...
    let pos =
      this.wot.wtOverlays.leftOverlay.getScrollPosition() -
      this.wot.wtOverlays.leftOverlay.getTableParentOffset();
    ...
    if (fixedColumnsLeft) {
      const fixedColumnsWidth = this.wot.wtOverlays.leftOverlay.sumCellSizes(
        0,
        fixedColumnsLeft,
      );
    ...
    if (
      this.wot.wtTable.holder.clientWidth !==
  ...
  createRenderCalculators(fastDraw = false) {
    ...
    if (runFastDraw) {
      ...
      const proposedColumnsVisibleCalculator =
        this.createColumnsCalculator(true);
  ...
  areAllProposedVisibleRowsAlreadyRendered(proposedRowsVisibleCalculator) {
    if (this.rowsVisibleCalculator) {
      if (
        proposedRowsVisibleCalculator.startRow <
          this.rowsRenderCalculator.startRow ||
        (proposedRowsVisibleCalculator.startRow ===
          this.rowsRenderCalculator.startRow &&
        ...
        proposedRowsVisibleCalculator.endRow >
          this.rowsRenderCalculator.endRow ||
        (proposedRowsVisibleCalculator.endRow ===
          this.rowsRenderCalculator.endRow &&
          proposedRowsVisibleCalculator.endRow <
            this.wot.getSetting('totalRows') - 1)
      ) {
        return false;
      }
      return true;
  ...
  areAllProposedVisibleColumnsAlreadyRendered(
    ...
    if (this.columnsVisibleCalculator) {
      if (
        proposedColumnsVisibleCalculator.startColumn <
          this.columnsRenderCalculator.startColumn ||
        (proposedColumnsVisibleCalculator.startColumn ===
          this.columnsRenderCalculator.startColumn &&
        ...
        proposedColumnsVisibleCalculator.endColumn >
          this.columnsRenderCalculator.endColumn ||
        (proposedColumnsVisibleCalculator.endColumn ===
          this.columnsRenderCalculator.endColumn &&
          proposedColumnsVisibleCalculator.endColumn <
            this.wot.getSetting('totalColumns') - 1)
      ) {
        return false;
      }
      return true;
