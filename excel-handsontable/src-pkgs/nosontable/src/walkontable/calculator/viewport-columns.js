...
class ViewportColumnsCalculator {
  ...
  calculate() {
    ...
    for (let i = 0; i < totalColumns; i++) {
      ...
      const compensatedViewportWidth =
        scrollOffset > 0 ? viewportWidth + 1 : viewportWidth;
  ...
  refreshStretching(totalWidth) {
    ...
    if (this.stretch === 'all' && remainingSize > 0) {
      ...
      this.stretchLastWidth =
        lastColumnWidth >= 0 ? lastColumnWidth : columnWidth;
    }
  }
  ...
  _getStretchedAllColumnWidth(column, baseWidth) {
    ...
    if (!this.stretchAllColumnsWidth[column]) {
      ...
      const newStretchedWidth = priv.stretchingColumnWidthFn(
        stretchedWidth,
        column,
      );
