...
class Border {
   ...
   */
  registerListeners() {
    this.eventManager.addEventListener(document.body, 'mousedown', () =>
      this.onMouseDown(),
    );
    ...
    for (let c = 0, len = this.main.childNodes.length; c < len; c++) {
      this.eventManager.addEventListener(
        ...
        'mouseenter',
        (event) => this.onMouseEnter(event, this.main.childNodes[c]),
      );
    }
  }
  ...
  onMouseEnter(event, parentElement) {
    ...
    function handler(handlerEvent) {
      if (isOutside(handlerEvent)) {
        _this.eventManager.removeEventListener(
          document.body,
          'mousemove',
          handler,
        );
  ...
  createBorders(settings) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      style.backgroundColor =
        this.settings[position] && this.settings[position].color
          ? this.settings[position].color
          : settings.border.color;
      style.height =
        this.settings[position] && this.settings[position].width
          ? `${this.settings[position].width}px`
  ...
  createMultipleSelectorHandles() {
    ...
    this.selectionHandles.topLeftHitArea.className =
      'topLeftSelectionHandle-HitArea';
    this.selectionHandles.bottomRight.className = 'bottomRightSelectionHandle';
    this.selectionHandles.bottomRightHitArea.className =
  ...
  isPartRange(row, col) {
    ...
    if (areaSelection.cellRange) {
      if (
        row !== areaSelection.cellRange.to.row ||
  ...
  updateMultipleSelectionHandlesPosition(row, col, top, left, width, height) {
    const handleWidth = parseInt(
      this.selectionHandles.styles.topLeft.width,
      10,
    );
    const hitAreaWidth = parseInt(
      this.selectionHandles.styles.topLeftHitArea.width,
      10,
    );
    ...
    if (
      row === this.wot.wtSettings.getSetting('fixedRowsTop') ||
  ...
  appear(corners) {
    ...
    const toTD = isMultiple
      ? this.wot.wtTable.getCell(new CellCoords(toRow, toColumn))
      : fromTD;
    ...
    cornerVisibleSetting =
      typeof cornerVisibleSetting === 'function'
        ? cornerVisibleSetting(this.settings.layerLevel)
        : cornerVisibleSetting;
    ...
    if (
      ...
      if (toColumn === this.wot.getSetting('totalColumns') - 1) {
        const toTdOffsetLeft = trimToWindow
          ? toTD.getBoundingClientRect().left
          : toTD.offsetLeft;
        const cornerRightEdge =
          toTdOffsetLeft +
          outerWidth(toTD) +
          parseInt(this.cornerDefaultStyle.width, 10) / 2;
        const cornerOverlappingContainer =
          cornerRightEdge >= innerWidth(trimmingContainer);
      ...
      if (toRow === this.wot.getSetting('totalRows') - 1) {
        const toTdOffsetTop = trimToWindow
          ? toTD.getBoundingClientRect().top
          : toTD.offsetTop;
        const cornerBottomEdge =
          toTdOffsetTop +
          outerHeight(toTD) +
          parseInt(this.cornerDefaultStyle.height, 10) / 2;
        const cornerOverlappingContainer =
          cornerBottomEdge >= innerHeight(trimmingContainer);
  ...
  isEntireColumnSelected(startRowIndex, endRowIndex) {
    return (
      startRowIndex === this.wot.wtTable.getFirstRenderedRow() &&
    ...
    if (rootHotElement.className.includes(entireSelectionClassname)) {
      ...
      if (startHeader && endHeader) {
        index =
          ...
          containerOffset[dimensionProperty] -
          1;
        dimension =
          endOffset[dimensionProperty] +
          dimensionFn(endHeader) -
          startHeaderOffset[dimensionProperty];
      }
