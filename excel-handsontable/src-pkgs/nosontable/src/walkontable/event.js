...
class Event {
  ...
  registerEvents() {
    this.eventManager.addEventListener(
      this.instance.wtTable.holder,
      'contextmenu',
      (event) => this.onContextMenu(event),
    );
    this.eventManager.addEventListener(
      this.instance.wtTable.TABLE,
      'mouseover',
  ...
  selectedCellWasTouched(touchTarget) {
    ...
    if (priv.selectedCellBeforeTouchEnd && coordsOfCellUnderFinger) {
      const [rowTouched, rowSelected] = [
        ...
        priv.selectedCellBeforeTouchEnd.from.row,
      ];
  ...
  onMouseOver(event) {
    ...
    if (td && td !== mainWOT.lastMouseOver && isChildOf(td, table)) {
      ...
      this.instance.getSetting(
        'onCellMouseOver',
        event,
        this.instance.wtTable.getCoords(td),
        td,
        this.instance,
      );
    }
  ...
  onMouseOut(event) {
    ...
    if (lastTD && lastTD !== nextTD && isChildOf(lastTD, table)) {
      this.instance.getSetting(
        'onCellMouseOut',
        event,
        this.instance.wtTable.getCoords(lastTD),
        lastTD,
        this.instance,
      );
    }
  ...
  onMouseUp(event) {
    ...
    if (
      cell.TD === priv.dblClickOrigin[0] &&
      cell.TD === priv.dblClickOrigin[1]
    ) {
  ...
  onTouchStart(event) {
    ...
    priv.selectedCellBeforeTouchEnd =
      this.instance.selections.getCell().cellRange;
  ...
  onTouchEnd(event) {
    ...
    if (
      this.selectedCellWasTouched(target) === false &&
