...
class Scroll {
  ...
  scrollViewport(coords, snapToTop, snapToRight, snapToBottom, snapToLeft) {
    const scrolledHorizontally = this.scrollViewportHorizontally(
      coords.col,
      snapToRight,
      snapToLeft,
    );
    const scrolledVertically = this.scrollViewportVertically(
      coords.row,
      snapToTop,
      snapToBottom,
    );
  ...
  scrollViewportHorizontally(column, snapToRight, snapToLeft) {
    ...
    const { fixedColumnsLeft, leftOverlay, totalColumns } =
      this._getVariables();
    let result = false;

    if (column >= 0 && column <= Math.max(totalColumns - 1, 0)) {
      if (
        column >= fixedColumnsLeft &&
        (column < this.getFirstVisibleColumn() || snapToLeft)
      ) {
  ...
  scrollViewportVertically(row, snapToTop, snapToBottom) {
    ...
    const { fixedRowsBottom, fixedRowsTop, topOverlay, totalRows } =
      this._getVariables();
    let result = false;
    ...
    if (row >= 0 && row <= Math.max(totalRows - 1, 0)) {
      if (
        row >= fixedRowsTop &&
        (row < this.getFirstVisibleRow() || snapToTop)
      ) {
        result = topOverlay.scrollTo(row);
      } else if (
        (row > this.getLastVisibleRow() && row < totalRows - fixedRowsBottom) ||
        snapToBottom
      ) {
   ...
   */
  getFirstVisibleRow() {
    const { topOverlay, wtTable, wtViewport, totalRows, fixedRowsTop } =
      this._getVariables();
    ...
    if (topOverlay.mainTableScrollableElement === window) {
      ...
      if (
        rootElementOffset.top + totalTableHeight - windowHeight <=
        windowScrollTop
      ) {
        ...
        rangeEachReverse(totalRows, 1, (row) => {
          ...
          if (
            rootElementOffset.top + totalTableHeight - rowsHeight <=
            windowScrollTop
          ) {
   ...
   */
  getLastVisibleRow() {
    const { topOverlay, wtTable, wtViewport, totalRows } = this._getVariables();
    ...
    if (topOverlay.mainTableScrollableElement === window) {
      ...
      if (rootElementOffset.top > windowScrollTop) {
        ...
        rangeEach(1, totalRows, (row) => {
          ...
          if (
            rootElementOffset.top + rowsHeight - windowScrollTop >=
            windowHeight
          ) {
  ...
  getFirstVisibleColumn() {
    const { leftOverlay, wtTable, wtViewport, totalColumns } =
      this._getVariables();
    ...
    if (leftOverlay.mainTableScrollableElement === window) {
      ...
      if (
        rootElementOffset.left + totalTableWidth - windowWidth <=
        windowScrollLeft
      ) {
        ...
        rangeEachReverse(totalColumns, 1, (column) => {
          ...
          if (
            rootElementOffset.left + totalTableWidth - columnsWidth <=
            windowScrollLeft
          ) {
    ...
    if (leftOverlay.mainTableScrollableElement === window) {
      ...
      if (rootElementOffset.left > windowScrollLeft) {
        ...
        rangeEach(1, totalColumns, (column) => {
          ...
          if (
            rootElementOffset.left + columnsWidth - windowScrollLeft >=
            windowWidth
          ) {
