...
class LeftOverlay extends Overlay {
  ...
  shouldBeRendered() {
    return !!(
      this.wot.getSetting('fixedColumnsLeft') ||
      this.wot.getSetting('rowHeaders').length
    );
  }
  ...
  adjustRootElementSize() {
    ...
    const scrollbarHeight =
      masterHolder.clientHeight === masterHolder.offsetHeight
        ? 0
        : getScrollbarWidth();
    const overlayRoot = this.clone.wtTable.holder.parentNode;
  ...
  adjustRootChildrenSize() {
    ...
    this.clone.wtTable.holder.style.height =
      this.clone.wtTable.holder.parentNode.style.height;
