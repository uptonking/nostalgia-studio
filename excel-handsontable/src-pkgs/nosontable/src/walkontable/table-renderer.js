...
class TableRenderer {
  ...
  render() {
    ...
    if (!this.wtTable.isWorkingOnClone()) {
      ...
      if (workspaceWidth !== this.wot.wtViewport.getWorkspaceWidth()) {
        ...
        rowHeaderWidthSetting = this.instance.getSetting(
        ...
        for (let i = firstRendered; i < lastRendered; i++) {
          ...
          this.COLGROUP.childNodes[
            renderedIndex + this.rowHeaderCount
  ...
  markOversizedRows() {
    ...
    const expectedTableHeight =
      rowCount * this.instance.wtSettings.settings.defaultRowHeight;
    ...
    if (
      expectedTableHeight === actualTableHeight &&
    ...
    while (rowCount) {
      rowCount -= 1;
      sourceRowIndex =
        this.instance.wtTable.rowFilter.renderedToSource(rowCount);
      ...
      if (
        (!previousRowHeight &&
          this.instance.wtSettings.settings.defaultRowHeight <
            rowInnerHeight) ||
  ...
  markOversizedColumnHeaders() {
    ...
    if (
      ...
      this.wot.wtViewport.hasOversizedColumnHeadersMarked[overlayName] ||
      this.wtTable.isWorkingOnClone()
    ) {
      return;
    }
    ...
    for (let i = 0; i < this.columnHeaderCount; i++) {
      for (
        let renderedColumnIndex = -1 * this.rowHeaderCount;
        renderedColumnIndex < columnCount;
        renderedColumnIndex++
      ) {
  ...
  adjustColumnHeaderHeights() {
    ...
    for (let i = 0, len = columnHeaders.length; i < len; i++) {
      if (oversizedColumnHeaders[i]) {
        ...
        children[i].childNodes[0].style.height =
          `${oversizedColumnHeaders[i]}px`;
      }
    }
  }
  ...
  markIfOversizedColumnHeader(col) {
    ...
    const columnHeaderHeightSetting =
      this.wot.getSetting('columnHeaderHeight') || [];
    ...
    while (level) {
      ...
      if (
        (!previousColHeaderHeight && defaultRowHeight < currentHeaderHeight) ||
        previousColHeaderHeight < currentHeaderHeight
      ) {
  ...
  renderCells(sourceRowIndex, TR, columnsToRender) {
    ...
    for (
      ...
      visibleColIndex < columnsToRender;
      visibleColIndex++
    ) {
      ...
      if (visibleColIndex === 0) {
        TD =
          TR.childNodes[
            this.columnFilter.sourceColumnToVisibleRowHeadedColumn(
              sourceColIndex,
            )
          ];
      } else {
  ...
  adjustColumnWidths(columnsToRender) {
    ...
    this.wot.wtViewport.columnsRenderCalculator.refreshStretching(
      this.wot.wtViewport.getViewportWidth() - scrollbarCompensation,
    );
    ...
    for (
      ...
      renderedColIndex < columnsToRender;
      renderedColIndex++
    ) {
      const width = this.wtTable.getStretchedColumnWidth(
        this.columnFilter.renderedToSource(renderedColIndex),
      );

      this.COLGROUP.childNodes[
        renderedColIndex + this.rowHeaderCount
      ].style.width = `${width}px`;
    }
  }
  ...
  createRow() {
    ...
    for (
      let visibleColIndex = 0;
      visibleColIndex < this.rowHeaderCount;
      visibleColIndex++
    ) {
  ...
  renderRowHeaders(row, TR) {
    for (
      let TH = TR.firstChild, visibleColIndex = 0;
      visibleColIndex < this.rowHeaderCount;
      visibleColIndex++
    ) {
  ...
  renderColumnHeaders() {
    ...
    for (let i = 0; i < this.columnHeaderCount; i++) {
      ...
      for (
        ...
        const sourceCol =
          this.columnFilter.renderedToSource(renderedColumnIndex);

        this.renderColumnHeader(
          i,
          sourceCol,
          TR.childNodes[renderedColumnIndex + this.rowHeaderCount],
        );
      }
    }
  }
