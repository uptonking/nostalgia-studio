import { closest, polymerWrap } from './helpers/dom/element';
...
function extendEvent(context, event) {
  ...
  if (isWebComponentSupportedNatively()) {
    ...
    hasOwnProperty(context, 'hot') ||
    context.isHotTableEnv ||
    context.wtTable
  ) {
    ...
    if (realTarget) {
      event.realTarget =
        fromElement.querySelector(componentName) || event.target;
    } else {
      event.realTarget = event.target;
    }
  }
