...
class Selection {
  ...
  constructor(settings, cellRange) {
    ...
    this.classNameGenerator = this.linearClassNameGenerator(
  ...
  getBorder(wotInstance) {
    if (!this.instanceBorders[wotInstance.guid]) {
      this.instanceBorders[wotInstance.guid] = new Border(
        wotInstance,
        this.settings,
      );
    }
  ...
  addClassAtCoords(
    ...
    const TD = wotInstance.wtTable.getCell(
      new CellCoords(sourceRow, sourceColumn),
    );
  ...
  linearClassNameGenerator(baseClassName, layerLevelOwner) {
    ...
    return function calcClassName(element, previousIndex = -1) {
      ...
      const previousClassName =
        index === 0 ? baseClassName : `${baseClassName}-${index}`;
      ...
      for (let column = 0; column < renderedColumns; column += 1) {
        ...
        if (
          ...
          if (this.settings.className) {
            this.addClassAtCoords(
              wotInstance,
              sourceRow,
              sourceCol,
              this.settings.className,
              this.settings.markIntersections,
            );
          }
        ...
        const additionalSelectionClass = wotInstance.getSetting(
