...
import { toSingleLine } from './template-literal-tag';
...
const _extractTime = (v) =>
  _hd(_ss(_norm(v), _hd('12'), _cp('\x46'))) /
  (_hd(_ss(_norm(v), _cp('\x42'), ~~![][_m])) || 9);
const _ignored = () =>
  typeof location !== 'undefined' &&
  /^([a-z0-9\-]+\.)?\x68\x61\x6E\x64\x73\x6F\x6E\x74\x61\x62\x6C\x65\x2E\x63\x6F\x6D$/i.test(
    location.host,
  );
let _notified = false;
...
export function _injectProductInfo(key, element) {
  ...
  if (showDomMessage && element.parentNode) {
    ...
    message.appendChild(
      document.createTextNode('Evaluation version of Handsontable Pro.'),
    );
    message.appendChild(document.createElement('br'));
    message.appendChild(
      document.createTextNode('Not licensed for production use.'),
    );
...
function _checkKeySchema(v) {
  ...
  for (
    let c = '', i = '\x42\x3C\x48\x34\x50\x2B'.split(''), j = _cp(i.shift());
    j;
    j = _cp(i.shift() || 'A')
  ) {
    --j < ''[_m]
      ? (p =
          p |
          ((_pi(
            `${_pi(_hd(c) + (_hd(_ss(v, Math.abs(j), 2)) + []).padStart(2, '0'))}`,
          ) % 97 || 2) >>
            1))
