...
export function getParent(element, level = 0) {
  ...
  while (elementToCheck !== null) {
    ...
    if (
      elementToCheck.host &&
      elementToCheck.nodeType === Node.DOCUMENT_FRAGMENT_NODE
    ) {
...
export function closest(element, nodes, until) {
  ...
  while (elementToCheck !== null && elementToCheck !== until) {
    if (
      ...
      (nodes.indexOf(elementToCheck.nodeName) > -1 ||
        nodes.indexOf(elementToCheck) > -1)
    ) {
...
export function isChildOf(child, parent) {
  ...
  if (typeof parent === 'string') {
    queriedParents = Array.prototype.slice.call(
...
export function isChildOfWebComponentTable(element) {
  ...
  function isHotTable(testElement) {
    return (
      testElement.nodeType === Node.ELEMENT_NODE &&
      testElement.nodeName === hotTableName.toUpperCase()
    );
  }
...
export function polymerWrap(element) {
  return typeof Polymer !== 'undefined' && typeof wrap === 'function'
    ? wrap(element)
    : element;
}
...
export function polymerUnwrap(element) {
  return typeof Polymer !== 'undefined' && typeof unwrap === 'function'
    ? unwrap(element)
    : element;
}
  ...
  _removeClass = function (element, classes) {
    ...
    while (className && className[len]) {
      ...
      _className = _className
        .replace(createClassNameRegExp(className[len]), ' ')
        .trim();
      len += 1;
    }
...
export function removeTextNodes(element, parent) {
  if (element.nodeType === 3) {
    ...
    ['TABLE', 'THEAD', 'TBODY', 'TFOOT', 'TR'].indexOf(element.nodeName) > -1
  ) {
...
export function getScrollableElement(element) {
  ...
  while (el && el.style && document.body !== el) {
    ...
    if (
      ...
      (props.indexOf(overflowY) !== -1 ||
        props.indexOf(overflow) !== -1 ||
        props.indexOf(computedOverflow) !== -1 ||
        props.indexOf(computedOverflowY) !== -1)
    ) {
      return el;
    }
    if (
      el.clientWidth <= el.scrollWidth + 1 &&
      (props.indexOf(overflowX) !== -1 ||
        props.indexOf(overflow) !== -1 ||
        props.indexOf(computedOverflow) !== -1 ||
        props.indexOf(computedOverflowX) !== -1)
    ) {
      return el;
    }
    el = el.parentNode;
...
export function outerHeight(elem) {
  if (
    hasCaptionProblem() &&
    elem.firstChild &&
    elem.firstChild.nodeName === 'CAPTION'
  ) {
...
export function setOverlayPosition(overlayElem, left, top) {
  if (isIE8() || isIE9()) {
    ...
    overlayElem.style['-webkit-transform'] =
...
export function getCssTransform(element) {
  ...
  if (element.style.transform && (transform = element.style.transform) !== '') {
    ...
    (transform = element.style['-webkit-transform']) !== ''
  ) {
...
export function isInput(element) {
  ...
  return (
    element &&
    (inputs.indexOf(element.nodeName) > -1 ||
...
export function isOutsideInput(element) {
  return (
    isInput(element) &&
    element.className.indexOf('handsontableInput') == -1 &&
    element.className.indexOf('copyPaste') == -1
  );
}
