...
class Table {
  ...
  constructor(wotInstance, table) {
    ...
    this.theadChildrenLength = this.THEAD.firstChild
      ? this.THEAD.firstChild.childNodes.length
      : 0;
    ...
    this.wot.wtSettings.settings.rowHeaderWidth = () =>
  ...
  draw(fastDraw) {
    ...
    if (runFastDraw) {
      ...
      if (
        Overlay.isOverlayTypeOf(this.wot.cloneOverlay, Overlay.CLONE_DEBUG) ||
        Overlay.isOverlayTypeOf(this.wot.cloneOverlay, Overlay.CLONE_TOP) ||
        Overlay.isOverlayTypeOf(
          this.wot.cloneOverlay,
        ...
        startRow = Math.max(
          totalRows - this.wot.getSetting('fixedRowsBottom'),
          0,
        );
      } else {
    ...
    if (!this.isWorkingOnClone()) {
      ...
      if (
        wtOverlays.bottomLeftCornerOverlay &&
        wtOverlays.bottomLeftCornerOverlay.clone
      ) {
  ...
  refreshSelections(fastDraw) {
    ...
    if (fastDraw) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (
          highlightRowClassName &&
          !classesToRemove.includes(highlightRowClassName)
        ) {
      ...
      const additionalClassesToRemove = this.wot.getSetting(
  ...
  getCell(coords) {
    ...
    const hookResult = this.wot.getSetting(
  ...
  getColumnHeader(col, level = 0) {
    ...
    if (TR) {
      return TR.childNodes[
        this.columnFilter.sourceColumnToVisibleRowHeadedColumn(col)
      ];
    }
  }
  ...
  getCoords(TD) {
    ...
    if (
      overlayContainsElement(Overlay.CLONE_TOP_LEFT_CORNER, cellElement) ||
      overlayContainsElement(Overlay.CLONE_TOP, cellElement)
    ) {
    ...
    if (
      overlayContainsElement(Overlay.CLONE_TOP_LEFT_CORNER, cellElement) ||
      overlayContainsElement(Overlay.CLONE_LEFT, cellElement)
    ) {
  ...
  isRowBeforeRenderedRows(row) {
    return (
      this.rowFilter && this.rowFilter.sourceToRendered(row) < 0 && row >= 0
    );
  }
  ...
  isRowAfterViewport(row) {
    return (
      this.rowFilter &&
      this.rowFilter.sourceToRendered(row) > this.getLastVisibleRow()
    );
  }
  ...
  isRowAfterRenderedRows(row) {
    return (
      this.rowFilter &&
      this.rowFilter.sourceToRendered(row) > this.getLastRenderedRow()
    );
  }
  ...
  isColumnBeforeViewport(column) {
    return (
      this.columnFilter &&
      this.columnFilter.sourceToRendered(column) < 0 &&
      column >= 0
    );
  }
  ...
  getRenderedColumnsCount() {
    ...
    if (this.wot.isOverlayName(Overlay.CLONE_DEBUG)) {
      ...
      this.wot.isOverlayName(Overlay.CLONE_LEFT) ||
  ...
  getRenderedRowsCount() {
    ...
    if (this.wot.isOverlayName(Overlay.CLONE_DEBUG)) {
      ...
      this.wot.isOverlayName(Overlay.CLONE_TOP) ||
      this.wot.isOverlayName(Overlay.CLONE_TOP_LEFT_CORNER)
    ) {
  ...
  allColumnsInViewport() {
    return (
      this.wot.getSetting('totalColumns') === this.getVisibleColumnsCount()
    );
  }
  ...
  getStretchedColumnWidth(sourceColumn) {
    ...
    if (calculator) {
      const stretchedWidth = calculator.getStretchedColumnWidth(
        sourceColumn,
        width,
      );
  ...
  _modifyRowHeaderWidth(rowHeaderWidthFactory) {
    let widths = isFunction(rowHeaderWidthFactory)
      ? rowHeaderWidthFactory()
      : null;

    if (Array.isArray(widths)) {
      widths = [...widths];
      widths[widths.length - 1] = this._correctRowHeaderWidth(
