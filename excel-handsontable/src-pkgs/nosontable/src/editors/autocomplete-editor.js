...
import { isPrintableChar, KEY_CODES } from '../helpers/unicode';
...
AutocompleteEditor.prototype.createElements = function (...args) {
  ...
  addClass(
    this.htContainer,
    window.navigator.platform.indexOf('Mac') === -1 ? '' : 'htMacScroll',
  );
};
...
function onBeforeKeyDown(event) {
  ...
  if (
    ...
    event.keyCode === KEY_CODES.INSERT
  ) {
    let timeOffset = 0;
...
AutocompleteEditor.prototype.open = function (...args) {
      ...
      if (cellValue && !allowHtml) {
        indexOfMatch =
          filteringCaseSensitive === true
            ? cellValue.indexOf(this.query)
            : cellValue.toLowerCase().indexOf(_this.query.toLowerCase());
  ...
  this.htEditor.view.wt.wtTable.holder.parentNode.style['padding-right'] =
    `${getScrollbarWidth() + 2}px`;
...
AutocompleteEditor.prototype.updateChoicesList = function (choicesList) {
  ...
  const orderByRelevanceLength = Array.isArray(orderByRelevance)
    ? orderByRelevance.length
    : 0;
...
AutocompleteEditor.prototype.flipDropdownIfNeeded = function () {
  ...
  const trimmingContainer = getTrimmingContainer(
    this.instance.view.wt.wtTable.TABLE,
  );
  ...
  this.limitDropdownIfNeeded(
    flipNeeded ? spaceAbove : spaceBelow,
    dropdownHeight,
  );
...
AutocompleteEditor.prototype.limitDropdownIfNeeded = function (
  spaceAvailable,
  dropdownHeight,
) {
  if (dropdownHeight > spaceAvailable) {
    ...
    do {
      lastRowHeight =
        this.htEditor.getRowHeight(i) ||
        this.htEditor.view.wt.wtSettings.settings.defaultRowHeight;
...
AutocompleteEditor.prototype.updateDropdownHeight = function () {
  const currentDropdownWidth =
    this.htEditor.getColWidth(0) + getScrollbarWidth() + 2;
...
AutocompleteEditor.prototype.finishEditing = function (
  restoreOriginalValue,
  ...args
) {
  HandsontableEditor.prototype.finishEditing.apply(this, [
    restoreOriginalValue,
    ...args,
  ]);
};
...
AutocompleteEditor.prototype.stripValuesIfNeeded = function (values) {
  ...
  const strippedValues = arrayMap(stringifiedValues, (value) =>
    allowHtml ? value : stripTags(value),
  );
