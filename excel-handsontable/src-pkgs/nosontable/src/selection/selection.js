...
import localHooks from '../mixins/local-hooks';
...
class Selection {
  constructor(settings, tableProps) {
    ...
    this.transformation.addLocalHook('beforeTransformStart', (...args) =>
      this.runLocalHooks('beforeModifyTransformStart', ...args),
    );
    this.transformation.addLocalHook('afterTransformStart', (...args) =>
      this.runLocalHooks('afterModifyTransformStart', ...args),
    );
    this.transformation.addLocalHook('beforeTransformEnd', (...args) =>
      this.runLocalHooks('beforeModifyTransformEnd', ...args),
    );
    this.transformation.addLocalHook('afterTransformEnd', (...args) =>
      this.runLocalHooks('afterModifyTransformEnd', ...args),
    );
    this.transformation.addLocalHook('insertRowRequire', (...args) =>
      this.runLocalHooks('insertRowRequire', ...args),
    );
    this.transformation.addLocalHook('insertColRequire', (...args) =>
      this.runLocalHooks('insertColRequire', ...args),
    );
  }
  ...
  setRangeStart(coords, multipleSelection, fragment = false) {
    ...
    if (
      !isMultipleMode ||
      (isMultipleMode && !isMultipleSelection && isUndefined(multipleSelection))
    ) {
  ...
  setRangeEnd(coords) {
    ...
    if (layerLevel < this.highlight.layerLevel) {
      arrayEach(
        this.highlight.getAreas(),
        (highlight) => void highlight.clear(),
      );
      arrayEach(
        this.highlight.getHeaders(),
        (highlight) => void highlight.clear(),
      );
      arrayEach(
        this.highlight.getActiveHeaders(),
        (highlight) => void highlight.clear(),
      );
    }
    ...
    if (
      this.highlight.isEnabledFor(AREA_TYPE) &&
  ...
  isMultiple() {
    const isMultipleListener = createObjectPropListener(
      !this.selectedRange.current().isSingle(),
    );
  ...
  transformStart(rowDelta, colDelta, force) {
    this.setRangeStart(
      this.transformation.transformStart(rowDelta, colDelta, force),
    );
  }
  ...
  isSelectedByColumnHeader(layerLevel = this.getLayerLevel()) {
    return layerLevel === -1
      ? this.selectedByColumnHeader.size > 0
  ...
  isCellCornerVisible() {
    return (
      this.settings.fillHandle &&
      !this.tableProps.isEditorOpened() &&
      !this.isMultiple()
    );
  }
  ...
  selectAll() {
    ...
    this.setRangeEnd(
      new CellCoords(
        this.tableProps.countRows() - 1,
        this.tableProps.countCols() - 1,
      ),
    );
  }
  ...
  selectCells(selectionRanges) {
    ...
    const isValid = !selectionRanges.some((selection) => {
      ...
      const _isValid =
        isValidCoord(rowStart, countRows) &&
  ...
  selectColumns(startColumn, endColumn = startColumn) {
    const start =
      typeof startColumn === 'string'
        ? this.tableProps.propToCol(startColumn)
        : startColumn;
    const end =
      typeof endColumn === 'string'
        ? this.tableProps.propToCol(endColumn)
        : endColumn;

    const countCols = this.tableProps.countCols();
    const isValid =
      isValidCoord(start, countCols) && isValidCoord(end, countCols);
  ...
  selectRows(startRow, endRow = startRow) {
    const countRows = this.tableProps.countRows();
    const isValid =
      isValidCoord(startRow, countRows) && isValidCoord(endRow, countRows);
