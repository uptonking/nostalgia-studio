...
export function detectSelectionType(selectionRanges, _callSymbol = rootCall) {
  if (_callSymbol !== rootCall && _callSymbol !== childCall) {
    throw new Error(
      'The second argument is used internally only and cannot be overwritten.',
    );
  }
  ...
  if (isArray) {
    ...
    if (selectionRanges.length === 0) {
      ...
      const isArrayType = !selectionRanges.some(
        (value, index) => !ARRAY_TYPE_PATTERN[index].includes(typeof value),
      );
 ...
 */
export function normalizeSelectionFactory(
  type,
  { keepDirection = false, propToCol } = {},
) {
...
export function transformSelectionToColumnDistance(selectionRanges) {
  ...
  if (
    selectionType === SELECTION_TYPE_UNRECOGNIZED ||
  ...
  arrayEach(selectionRanges, (selection) => {
    ...
    arrayEach(
      Array.from(new Array(amount), (_, i) => columnStart + i),
      (index) => {
  ...
  const normalizedColumnRanges = arrayReduce(
    orderedIndexes,
    (acc, visualColumnIndex, index, array) => {
...
export function transformSelectionToRowDistance(selectionRanges) {
  ...
  arrayEach(selectionRanges, (selection) => {
    ...
    arrayEach(
      Array.from(new Array(amount), (_, i) => rowStart + i),
      (index) => {
        if (!unorderedIndexes.has(index)) {
          unorderedIndexes.add(index);
        }
  ...
  // Sort indexes in ascending order to easily detecting non-consecutive columns.
  const orderedIndexes = Array.from(unorderedIndexes).sort((a, b) => a - b);
  const normalizedRowRanges = arrayReduce(
    orderedIndexes,
    (acc, rowIndex, index, array) => {
