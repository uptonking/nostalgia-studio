import {
  isRightClick as isRightClickEvent,
  isLeftClick as isLeftClickEvent,
...
export function mouseDown({
  ...
  const currentSelection = selection.isSelected()
    ? selection.getSelectedRange().current()
    : null;
  ...
  if (isShiftKey && currentSelection) {
    if (coords.row >= 0 && coords.col >= 0 && !controller.cells) {
      ...
      selection.setRangeEnd(
        new CellCoords(currentSelection.to.row, coords.col),
      );
    } else if (selectedRow && coords.col < 0 && !controller.row) {
      selection.setRangeEnd(
        new CellCoords(coords.row, currentSelection.to.col),
      );
    } else if (
      ((!selectedCorner && !selectedRow && coords.col < 0) ||
        (selectedCorner && coords.col < 0)) &&
      !controller.row
    ) {
      selection.selectRows(currentSelection.from.row, coords.row);
    } else if (
      ((!selectedCorner && !selectedRow && coords.row < 0) ||
        (selectedRow && coords.row < 0)) &&
      !controller.column
    ) {
    ...
    const performSelection =
      isLeftClick || (isRightClick && allowRightClickSelection);
