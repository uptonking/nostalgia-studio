...
class Transformation {
  ...
  transformStart(rowDelta, colDelta, force) {
    ...
    if (highlightCoords.row + rowDelta > totalRows - 1) {
      if (
        ...
        !(
          fixedRowsBottom &&
          highlightCoords.row >= totalRows - fixedRowsBottom - 1
        )
      ) {
        ...
        delta.col =
          highlightCoords.col + delta.col === totalCols - 1 ? 1 - totalCols : 1;
      }
    } else if (
      autoWrapCol &&
      highlightCoords.row + delta.row < 0 &&
      highlightCoords.col + delta.col >= 0
    ) {
    ...
    if (highlightCoords.col + delta.col > totalCols - 1) {
      if (force && minSpareCols > 0) {
        ...
        delta.row =
          highlightCoords.row + delta.row === totalRows - 1 ? 1 - totalRows : 1;
    ...
    const coords = new CellCoords(
      highlightCoords.row + delta.row,
      highlightCoords.col + delta.col,
    );
  ...
  transformEnd(rowDelta, colDelta) {
    ...
    const coords = new CellCoords(
      cellRange.to.row + delta.row,
      cellRange.to.col + delta.col,
    );
    let rowTransformDir = 0;
    let colTransformDir = 0;
