...
import { getLanguageDictionary } from './dictionaries-manager';
import { getPhraseFormatters } from './phrase-formatters';
...
export function getTranslatedPhrase(
  ...
  const formattedPhrase = getFormattedPhrase(
    phrasePropositions,
    argumentsForFormatters,
  );
...
function getFormattedPhrase(phrasePropositions, argumentsForFormatters) {
  ...
  arrayEach(getPhraseFormatters(), (formatter) => {
    formattedPhrasePropositions = formatter(
      phrasePropositions,
      argumentsForFormatters,
    );
  });
