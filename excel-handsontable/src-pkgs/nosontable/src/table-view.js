...
import Walkontable from './walkontable';
import { handleMouseEvent } from './selection';
...
function TableView(instance) {
  ...
  this.eventManager.addEventListener(
    ...
    (event) => {
      if (
        ...
        !that.isTextSelectionAllowed(event.target)
      ) {
  ...
  this.eventManager.addEventListener(
    ...
    (event) => {
      ...
      if (
        isOutsideInput(document.activeElement) ||
        (!instance.selection.isSelected() && !isRightClick(event))
      ) {
        instance.unlisten();
      }
    },
  ...
  this.eventManager.addEventListener(
    ...
    (event) => {
      ...
      if (next === instance.view.wt.wtTable.holder) {
        ...
        if (
          document.elementFromPoint(eventX + scrollbarWidth, eventY) !==
            instance.view.wt.wtTable.holder ||
          document.elementFromPoint(eventX, eventY + scrollbarWidth) !==
            instance.view.wt.wtTable.holder
        ) {
          return;
        }
      ...
      const outsideClickDeselects =
        typeof that.settings.outsideClickDeselects === 'function'
  ...
  const walkontableConfig = {
    ...
    cellRenderer(row, col, TD) {
      ...
      that.instance.getCellRenderer(cellProperties)(
        that.instance,
        TD,
        row,
        col,
    ...
    onBeforeRemoveCellClassNames: () =>
      instance.runHooks('beforeRemoveCellClassNames'),
    ...
    onBeforeStretchingColumnWidth: (stretchedWidth, column) =>
      instance.runHooks('beforeStretchingColumnWidth', stretchedWidth, column),
    onModifyRowHeaderWidth: (rowHeaderWidth) =>
      instance.runHooks('modifyRowHeaderWidth', rowHeaderWidth),
    onModifyGetCellCoords: (row, column, topmost) =>
      instance.runHooks('modifyGetCellCoords', row, column, topmost),
...
TableView.prototype.isTextSelectionAllowed = function (el) {
  ...
  const isChildOfTableBody = isChildOf(
    el,
  ...
  if (
    this.settings.fragmentSelection === 'cell' &&
 ...
 * @returns {Boolean}
 */
TableView.prototype.scrollViewport = function (
  coords,
  snapToTop,
  snapToRight,
  snapToBottom,
 ...
 * @returns {Boolean}
 */
TableView.prototype.scrollViewportHorizontally = function (
  column,
  snapToRight,
  snapToLeft,
) {
 ...
 * @returns {Boolean}
 */
TableView.prototype.scrollViewportVertically = function (
  row,
  snapToTop,
  snapToBottom,
) {
...
TableView.prototype.appendRowHeader = function (row, TH) {
  if (TH.firstChild) {
    ...
    this.updateCellHeader(
      container.querySelector('.rowHeader'),
      row,
...
TableView.prototype.updateCellHeader = function (element, index, content) {
  ...
  if (element.parentNode) {
    if (hasClass(element, 'colHeader')) {
      renderedIndex =
        parentOverlay.wtTable.columnFilter.sourceToRendered(index);
