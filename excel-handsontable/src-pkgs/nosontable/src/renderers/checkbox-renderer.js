...
function checkboxRenderer(
  ...
  if (
    ...
    equalsIgnoreCase(value, cellProperties.checkedTemplate)
  ) {
    input.checked = true;
  } else if (
  ...
  if (!badValue && labelOptions) {
    ...
    if (labelOptions.value) {
      labelText =
        typeof labelOptions.value === 'function'
          ? labelOptions.value.call(this, row, col, prop, value)
          : labelOptions.value;
  ...
  function onBeforeKeyDown(event) {
    ...
    if (
      isKeyCode(`${toggleKeys}|${switchOffKeys}`) &&
  ...
  function changeSelectedCheckboxesState(uncheckCheckbox = false) {
    ...
    for (let visualRow = startRow; visualRow <= endRow; visualRow += 1) {
      for (
        ...
        visualColumn <= endColumn;
        visualColumn += 1
      ) {
        const cachedCellProperties = instance.getCellMeta(
          visualRow,
          visualColumn,
        );
        ...
        if (uncheckCheckbox === false) {
          if (
            [
              ...
              cachedCellProperties.checkedTemplate.toString(),
            ].includes(dataAtCell)
          ) {
            changes.push([
              visualRow,
  ...
  function eachSelectedCheckboxCell(callback) {
    ...
    for (
      let visualRow = topLeft.row;
      visualRow <= bottomRight.row;
      visualRow++
    ) {
      for (
        let visualColumn = topLeft.col;
        visualColumn <= bottomRight.col;
        visualColumn++
      ) {
        const cachedCellProperties = instance.getCellMeta(
          visualRow,
...
function registerEvents(instance) {
  ...
  if (!eventManager) {
    ...
    eventManager.addEventListener(instance.rootElement, 'mouseup', (event) =>
