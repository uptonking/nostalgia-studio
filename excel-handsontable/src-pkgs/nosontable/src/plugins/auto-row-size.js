...
class AutoRowSize extends BasePlugin {
  ...
  constructor(hotInstance) {
    ...
    this.addHook('beforeRowResize', (row, size, isDblClick) =>
      this.onBeforeRowResize(row, size, isDblClick),
    );
  }
  ...
  isEnabled() {
    return (
      this.hot.getSettings().autoRowSize === true ||
      isObject(this.hot.getSettings().autoRowSize)
    );
  }
  ...
  enablePlugin() {
    ...
    this.addHook('beforeRowMove', (rowStart, rowEnd) =>
      this.onBeforeRowMove(rowStart, rowEnd),
    );
    this.addHook('modifyRowHeight', (height, row) =>
      this.getRowHeight(row, height),
    );
  ...
  calculateRowsHeight(
    ...
    rangeEach(rowsRange.from, rowsRange.to, (row) => {
      ...
      if (force || this.heights[row] === void 0) {
        ...
        arrayEach(samples, ([rowIndex, sample]) =>
          this.ghostTable.addRow(rowIndex, sample),
        );
      }
    });
