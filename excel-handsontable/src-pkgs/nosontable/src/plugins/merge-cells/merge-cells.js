...
class MergeCells extends BasePlugin {
  ...
  enablePlugin() {
    ...
    this.addHook('modifyTransformStart', (...args) =>
      this.onModifyTransformStart(...args),
    );
    this.addHook('afterModifyTransformStart', (...args) =>
      this.onAfterModifyTransformStart(...args),
    );
    this.addHook('modifyTransformEnd', (...args) =>
      this.onModifyTransformEnd(...args),
    );
    this.addHook('modifyGetCellCoords', (...args) =>
      this.onModifyGetCellCoords(...args),
    );
    this.addHook('beforeSetRangeEnd', (...args) =>
      this.onBeforeSetRangeEnd(...args),
    );
    this.addHook('afterIsMultipleSelection', (...args) =>
      this.onAfterIsMultipleSelection(...args),
    );
    this.addHook('afterRenderer', (...args) => this.onAfterRenderer(...args));
    this.addHook('afterContextMenuDefaultOptions', (...args) =>
      this.addMergeActionsToContextMenu(...args),
    );
    this.addHook('afterGetCellMeta', (...args) =>
      this.onAfterGetCellMeta(...args),
    );
    this.addHook('afterViewportRowCalculatorOverride', (...args) =>
      this.onAfterViewportRowCalculatorOverride(...args),
    );
    this.addHook('afterViewportColumnCalculatorOverride', (...args) =>
      this.onAfterViewportColumnCalculatorOverride(...args),
    );
    this.addHook('modifyAutofillRange', (...args) =>
      this.onModifyAutofillRange(...args),
    );
    ...
    this.addHook('beforeDrawBorders', (...args) =>
      this.onBeforeDrawAreaBorders(...args),
    );
    this.addHook('afterDrawSelection', (...args) =>
      this.onAfterDrawSelection(...args),
    );
    this.addHook('beforeRemoveCellClassNames', (...args) =>
      this.onBeforeRemoveCellClassNames(...args),
    );
  ...
  validateSetting(setting) {
    ...
    if (MergedCellCoords.containsNegativeValues(setting)) {
      ...
      MergedCellCoords.isOutOfBounds(
        setting,
        this.hot.countRows(),
        this.hot.countCols(),
      )
  ...
  generateFromSettings(settings) {
    if (Array.isArray(settings)) {
      ...
      arrayEach(settings, (setting) => {
        ...
        const rangeEnd = new CellCoords(
          setting.row + setting.rowspan - 1,
      ...
      populationArgumentsList = populationArgumentsList.filter(
        (value) => value !== true,
      );

      const bulkPopulationData = this.getBulkCollectionData(
        populationArgumentsList,
      );
  ...
  getBulkCollectionData(populationArgumentsList) {
    const populationDataRange = this.getBulkCollectionDataRange(
      populationArgumentsList,
    );
    ...
    arrayEach(populationArgumentsList, (mergedCellArguments) => {
      const [mergedCellRowIndex, mergedCellColumnIndex, mergedCellData] =
        mergedCellArguments;

      arrayEach(mergedCellData, (mergedCellRow, rowIndex) => {
        arrayEach(mergedCellRow, (mergedCellElement, columnIndex) => {
          newDataAtRange[
            mergedCellRowIndex - populationDataRange[0] + rowIndex
          ][mergedCellColumnIndex - populationDataRange[1] + columnIndex] =
            mergedCellElement;
        });
      });
    });
  ...
  getBulkCollectionDataRange(populationArgumentsList) {
    ...
    arrayEach(populationArgumentsList, (mergedCellArguments) => {
      ...
      start[1] = Math.min(mergedCellColumn, start[1]);
        ...
        mergedCellColumn + mergedCellData[0].length - 1,
        end[1],
      );
    });
  ...
  mergeRange(cellRange, auto = false, preventPopulation = false) {
    ...
    rangeEach(0, mergeParent.rowspan - 1, (i) => {
      rangeEach(0, mergeParent.colspan - 1, (j) => {
        ...
        if (i === 0 && j === 0) {
          clearedValue = this.hot.getDataAtCell(
            mergeParent.row,
            mergeParent.col,
          );
        } else {
  ...
  unmergeRange(cellRange, auto = false) {
    ...
    arrayEach(mergedCells, (currentCollection) => {
      this.mergedCellsCollection.remove(
      ...
      rangeEach(0, currentCollection.rowspan - 1, (i) => {
        rangeEach(0, currentCollection.colspan - 1, (j) => {
          this.hot.removeCellMeta(
            currentCollection.row + i,
            currentCollection.col + j,
            'hidden',
          );
        });
      });
  ...
  toggleMerge(cellRange) {
    const mergedCell = this.mergedCellsCollection.get(
      cellRange.from.row,
      cellRange.from.col,
    );
    const mergedCellCoversWholeRange =
      mergedCell.row === cellRange.from.row &&
  ...
  onAfterIsMultipleSelection(isMultiple) {
    if (isMultiple) {
      ...
      for (let group = 0; group < mergedCells.length; group += 1) {
        if (
          selectionRange.highlight.row === mergedCells[group].row &&
          selectionRange.highlight.col === mergedCells[group].col &&
          selectionRange.to.row ===
            mergedCells[group].row + mergedCells[group].rowspan - 1 &&
          selectionRange.to.col ===
            mergedCells[group].col + mergedCells[group].colspan - 1
        ) {
          return false;
        }
      }
  ...
  onModifyTransformStart(delta) {
    ...
    const currentPosition = new CellCoords(
      currentlySelectedRange.highlight.row,
      currentlySelectedRange.highlight.col,
    );
    const mergedParent = this.mergedCellsCollection.get(
      currentPosition.row,
      currentPosition.col,
    );
    ...
    if (mergedParent) {
      ...
      const mergeRange = new CellRange(
        mergeTopLeft,
        mergeTopLeft,
        mergeBottomRight,
      );
      ...
      newDelta.row = priv.lastDesiredCoords.row
        ? priv.lastDesiredCoords.row - currentPosition.row
        : newDelta.row;
      newDelta.col = priv.lastDesiredCoords.col
        ? priv.lastDesiredCoords.col - currentPosition.col
        : newDelta.col;
    ...
    const nextParentIsMerged = this.mergedCellsCollection.get(
      nextPosition.row,
      nextPosition.col,
    );
  ...
  onModifyTransformEnd(delta) {
    ...
    const newSelectionRange =
      this.selectionCalculations.getUpdatedSelectionRange(
        currentSelectionRange,
        delta,
      );
    let tempDelta = clone(newDelta);

    const mergedCellsWithinRange = this.mergedCellsCollection.getWithinRange(
      newSelectionRange,
      true,
    );
    ...
    do {
      ...
      arrayEach(mergedCellsWithinRange, (mergedCell) => {
        this.selectionCalculations.snapDelta(
          newDelta,
          currentSelectionRange,
          mergedCell,
        );
  ...
  onBeforeSetRangeEnd(coords) {
    ...
    selRange.highlight = new CellCoords(
      selRange.highlight.row,
      selRange.highlight.col,
    ...
    if (
      (selRange.from.row === 0 &&
        selRange.to.row === this.hot.countRows() - 1) ||
      (selRange.from.col === 0 && selRange.to.col === this.hot.countCols() - 1)
    ) {
      return;
    }
    ...
    do {
      ...
      for (
        let i = 0;
        i < this.mergedCellsCollection.mergedCells.length;
        i += 1
      ) {
  ...
  onModifyAutofillRange(drag, select) {
    ...
    if (
      this.autofillCalculations.dragAreaOverlapsCollections(
        select,
        dragArea,
        dragDirection,
      )
    ...
    dragArea = this.autofillCalculations.snapDragArea(
      select,
      dragArea,
      dragDirection,
  ...
  onBeforeDrawAreaBorders(corners, className) {
    if (className && className === 'area') {
      ...
      const mergedCellsWithinRange =
        this.mergedCellsCollection.getWithinRange(selectedRange);

      arrayEach(mergedCellsWithinRange, (mergedCell) => {
        if (
          selectedRange.getBottomRightCorner().row ===
            mergedCell.getLastRow() &&
          selectedRange.getBottomRightCorner().col ===
  ...
  onAfterModifyTransformStart(coords, rowTransformDir, colTransformDir) {
    ...
    const mergedCellAtCoords = this.mergedCellsCollection.get(
      coords.row,
      coords.col,
    );
    ...
    const mergedCellOnBottomEdge =
      mergedCellAtCoords.row + mergedCellAtCoords.rowspan - 1 ===
      this.hot.countRows() - 1;
    const mergedCellOnTopEdge = mergedCellAtCoords.row === 0;
    const mergedCellOnRightEdge =
      mergedCellAtCoords.col + mergedCellAtCoords.colspan - 1 ===
      this.hot.countCols() - 1;
    const mergedCellOnLeftEdge = mergedCellAtCoords.col === 0;

    if (
      (goingDown && mergedCellOnBottomEdge) ||
      (goingUp && mergedCellOnTopEdge) ||
      (goingRight && mergedCellOnRightEdge) ||
  ...
  onAfterDrawSelection(
    ...
    return this.selectionCalculations.getSelectedMergedCellClassName(
      currentRow,
      currentColumn,
      cornersOfSelection,
      layerLevel,
