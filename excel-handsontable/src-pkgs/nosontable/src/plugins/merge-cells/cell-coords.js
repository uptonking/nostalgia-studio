...
class MergedCellCoords {
  ...
  static containsNegativeValues(mergedCellInfo) {
    return (
      mergedCellInfo.row < 0 ||
      mergedCellInfo.col < 0 ||
  ...
  shift(shiftVector, indexOfChange) {
    ...
    const shiftedIndex =
      indexOfChange + Math.abs(shiftVector[0] || shiftVector[1]) - 1;
    ...
    if (shiftValue > 0) {
      ...
      if (changeStart <= mergeStart && changeEnd >= mergeEnd) {
        ...
        mergeStart <= changeStart &&
        mergeEnd >= changeStart &&
        mergeEnd < changeEnd
      ) {
  ...
  isFarther(mergedCell, direction) {
    ...
    if (direction === 'down') {
      return (
        mergedCell.row + mergedCell.rowspan - 1 < this.row + this.rowspan - 1
      );
    } else if (direction === 'up') {
      return mergedCell.row > this.row;
    } else if (direction === 'right') {
      return (
        mergedCell.col + mergedCell.colspan - 1 < this.col + this.colspan - 1
      );
