...
class AutofillCalculations {
  ...
  correctSelectionAreaSize(selectionArea) {
    if (
      selectionArea[0] === selectionArea[2] &&
      selectionArea[1] === selectionArea[3]
    ) {
  ...
  getDirection(selectionArea, finalArea) {
    ...
    if (
      finalArea[0] === selectionArea[0] &&
      finalArea[1] === selectionArea[1] &&
      finalArea[3] === selectionArea[3]
    ) {
      direction = 'down';
    } else if (
      finalArea[2] === selectionArea[2] &&
      finalArea[1] === selectionArea[1] &&
      finalArea[3] === selectionArea[3]
    ) {
      direction = 'up';
  ...
  snapDragArea(baseArea, dragArea, dragDirection, foundMergedCells) {
    ...
    const fullCycle = verticalDirection
      ? baseAreaEndRow - baseAreaStartRow + 1
      : baseAreaEndColumn - baseAreaStartColumn + 1;
    const fulls = Math.floor(fillSize / fullCycle) * fullCycle;
    const partials = fillSize - fulls;
    const farthestCollection = this.getFarthestCollection(
      baseArea,
      dragArea,
      dragDirection,
      foundMergedCells,
    ...
    if (farthestCollection) {
      if (dragDirection === 'down') {
        const fill =
          farthestCollection.row +
          farthestCollection.rowspan -
          baseAreaStartRow -
          partials;
        ...
        const fill =
          farthestCollection.col +
          farthestCollection.colspan -
          baseAreaStartColumn -
          partials;
  ...
  getFarthestCollection(baseArea, dragArea, direction, mergedCellArray) {
    ...
    const baseStart = verticalDirection
      ? baseAreaStartRow
      : baseAreaStartColumn;
  ...
  recreateAfterDataPopulation(changes) {
    ...
    const inBounds = (current, offset) => {
      switch (dragDirection) {
        ...
        case 'right':
          return (
            current.col + current.colspan - 1 + offset <= fillRange.to.column
          );
        default:
          return null;
      }
