...
class SelectionCalculations {
  ...
  getUpdatedSelectionRange(oldSelectionRange, delta) {
    return new CellRange(
      ...
      new CellCoords(
        oldSelectionRange.to.row + delta.row,
        oldSelectionRange.to.col + delta.col,
      ),
    );
  }
  ...
  getSelectedMergedCellClassName(
    ...
    if (
      ...
      const isMergedCellParent =
        this.plugin.mergedCellsCollection.isMergedParent(
          currentRow,
          currentColumn,
        );
      ...
      const mergedCell = this.plugin.mergedCellsCollection.get(
        currentRow,
        currentColumn,
      );
      ...
      if (
        mergedCell.row + mergedCell.rowspan - 1 <= endRow &&
        mergedCell.col + mergedCell.colspan - 1 <= endColumn
      ) {
        return `${this.fullySelectedMergedCellClassName}-${layerLevel}`;
      } else if (
        this.plugin.selectionCalculations.isMergeCellFullySelected(
          mergedCell,
  ...
  isMergeCellFullySelected(mergedCell, selectionRangesArray) {
    ...
    for (let r = 0; r < mergedCell.rowspan; r += 1) {
      for (let c = 0; c < mergedCell.colspan; c += 1) {
        mergedCellIndividualCoords.push(
          new CellCoords(mergedCell.row + r, mergedCell.col + c),
        );
      }
    }
    ...
    for (let i = 0; i < mergedCellIndividualCoords.length; i += 1) {
      ...
      for (let s = 0; s < selectionRangesArray.length; s += 1) {
        insideSelections[s] = selectionRangesArray[s].includes(
          mergedCellIndividualCoords[i],
        );
      }
