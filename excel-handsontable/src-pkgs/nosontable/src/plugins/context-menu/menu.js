...
import { hasOwnProperty, mixin } from '../../helpers/object';
import { KEY_CODES } from '../../helpers/unicode';
import localHooks from '../../mixins/local-hooks';
import Cursor from './cursor';
import { SEPARATOR } from './predefined-items';
import {
  filterSeparators,
  hasSubMenu,
  isDisabled,
...
class Menu {
  ...
  open() {
      ...
      columns: [
        {
          ...
          renderer: (hot, TD, row, col, prop, value) =>
            this.menuItemRenderer(hot, TD, row, col, prop, value),
        },
      ],
      renderAllRows: true,
    ...
    this.hotMenu.addHook('afterSelection', (...args) =>
      this.onAfterSelection(...args),
    );
    this.hotMenu.init();
    this.hotMenu.listen();
  ...
  executeCommand(event) {
    ...
    const selectedItem = this.hotMenu.getSourceDataAtRow(
      this.hotMenu.getSelectedLast()[0],
    );
    ...
    if (
      ...
      (typeof selectedItem.disabled === 'function' &&
        selectedItem.disabled.call(this.hot) === true) ||
  ...
  selectNextCell(row, col) {
    const nextRow = row + 1;
    const cell =
      nextRow < this.hotMenu.countRows()
        ? this.hotMenu.getCell(nextRow, col)
        : null;
  ...
  menuItemRenderer(hot, TD, row, col, prop, value) {
    ...
    const itemIsSeparator = (itemToTest) =>
      new RegExp(SEPARATOR, 'i').test(itemToTest.name);
    const itemIsDisabled = (itemToTest) =>
      itemToTest.disabled === true ||
      (typeof itemToTest.disabled === 'function' &&
  ...
  onAfterInit() {
    ...
    const realHeight = arrayReduce(
      data,
      (accumulator, value) => accumulator + (value.name === SEPARATOR ? 1 : 26),
      0,
    );
  ...
  onDocumentMouseDown(event) {
    ...
    if (
      ...
      this.hotMenu &&
      !isChildOf(event.target, this.hotMenu.rootElement)
    ) {
      this.close(true);

      // Automatically close menu when clicked element is not belongs to menu or submenu (not necessarily to itself)
    } else if (
      (this.isAllSubMenusClosed() || this.isSubMenu()) &&
      !isChildOf(event.target, '.htMenu') &&
