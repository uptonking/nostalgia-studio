...
class ManualColumnFreeze extends BasePlugin {
  ...
  enablePlugin() {
    ...
    this.addHook('beforeColumnMove', (rows, target) =>
      this.onBeforeColumnMove(rows, target),
    );
  ...
  freezeColumn(column) {
    ...
    if (
      settings.fixedColumnsLeft === this.hot.countCols() ||
      column <= settings.fixedColumnsLeft - 1
    ) {
