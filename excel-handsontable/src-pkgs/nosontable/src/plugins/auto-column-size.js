...
class AutoColumnSize extends BasePlugin {
  ...
  constructor(hotInstance) {
        ...
        if (labelValue) {
          labelText =
            typeof labelValue === 'function'
              ? labelValue(row, column, this.hot.colToProp(column), cellValue)
              : labelValue;
    ...
    this.addHook('beforeColumnResize', (col, size, isDblClick) =>
      this.onBeforeColumnResize(col, size, isDblClick),
    );
  }
  ...
  isEnabled() {
    return (
      this.hot.getSettings().autoColumnSize !== false &&
  ...
  calculateColumnsWidth(
    ...
    const columnsRange =
      typeof colRange === 'number'
        ? { from: colRange, to: colRange }
        : colRange;
    const rowsRange =
      typeof rowRange === 'number'
        ? { from: rowRange, to: rowRange }
        : rowRange;
    ...
    rangeEach(columnsRange.from, columnsRange.to, (col) => {
      if (
        ...
        arrayEach(samples, ([column, sample]) =>
          this.ghostTable.addColumn(column, sample),
        );
      }
    });
  ...
  findColumnsWhereHeaderWasChanged() {
    ...
    const changedColumns = arrayReduce(
      columnHeaders,
      (acc, columnTitle, physicalColumn) => {
        const cachedColumnsLength = cachedColumnHeaders.length;

        if (
          cachedColumnsLength - 1 < physicalColumn ||
  ...
  onBeforeChange(changes) {
    const changedColumns = arrayMap(changes, ([, column]) =>
      this.hot.propToCol(column),
    );
