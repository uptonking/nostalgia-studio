...
class ManualRowMove extends BasePlugin {
  ...
  enablePlugin() {
    ...
    this.addHook('beforeRemoveRow', (index, amount) =>
      this.onBeforeRemoveRow(index, amount),
    );
    ...
    this.addHook('beforeColumnSort', (column, order) =>
      this.onBeforeColumnSort(column, order),
    );
  ...
  moveRows(rows, target) {
    ...
    const beforeMoveHook = this.hot.runHooks(
      'beforeRowMove',
      visualRows,
      target,
    );
  ...
  refreshPositions() {
    ...
    let tdOffsetTop =
      this.hot.view.THEAD.offsetHeight + this.getRowsHeight(0, coords.row);
    const mouseOffsetTop =
      priv.target.eventPageY - rootElementOffset.top + wtTable.holder.scrollTop;
    ...
    if (
      mouseOffsetTop + backlightElemHeight + backlightElemMarginTop >=
      hiderHeight
    ) {
    ...
    if (this.hot.view.wt.wtOverlays.topOverlay) {
      topOverlayHeight =
        this.hot.view.wt.wtOverlays.topOverlay.clone.wtTable.TABLE.offsetHeight;
    }

    if (
      coords.row >= fixedRows &&
      guidelineTop - wtTable.holder.scrollTop < topOverlayHeight
    ) {
  ...
  onBeforeOnCellMouseDown(event, coords, TD, blockCalculations) {
    ...
    if (coords.col < 0 && coords.row >= start && coords.row <= end) {
      ...
      const leftPos =
        ...
        this.hot.view.wt.wtViewport.getRowHeaderWidth();

      this.backlight.setPosition(null, leftPos);
      this.backlight.setSize(
        wtTable.hider.offsetWidth - leftPos,
        this.getRowsHeight(start, end + 1),
      );
      this.backlight.setOffset(
        (this.getRowsHeight(start, coords.row) + event.layerY) * -1,
        null,
      );
  ...
  onMouseUp() {
    ...
    if (!priv.disallowMoving) {
      const selectionStart = this.rowsMapper.getIndexByValue(
        priv.rowsToMove[0],
      );
      const selectionEnd = this.rowsMapper.getIndexByValue(
        priv.rowsToMove[rowsLen - 1],
      );
    ...
    if (index !== false) {
      ...
      rangeEach(index, index + amount - 1, (removedIndex) => {
        this.removedRows.push(
          this.hot.runHooks('modifyRow', removedIndex, this.pluginName),
        );
      });
    }
  }
