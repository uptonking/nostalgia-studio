...
class Comments extends BasePlugin {
  ...
  enablePlugin() {
    ...
    this.displaySwitch.addLocalHook('show', (row, col) =>
      this.showAtCell(row, col),
    );
   ...
   */
  registerListeners() {
    this.eventManager.addEventListener(document, 'mouseover', (event) =>
      this.onMouseOver(event),
    );
    this.eventManager.addEventListener(document, 'mousedown', (event) =>
    ...
    this.eventManager.addEventListener(
      this.editor.getInputElement(),
      'mouseup',
      (event) => this.onEditorMouseUp(event),
    );
  }
  ...
  targetIsCellWithComment(event) {
    ...
    return !!(
      closestCell &&
      hasClass(closestCell, 'htCommentCell') &&
  ...
  setComment(value) {
    if (!this.range.from) {
      throw new Error(
        'Before using this method, first set cell range (hot.getPlugin("comment").setRange())',
      );
    }
  ...
  show() {
    ...
    this.editor.setValue(
      meta[META_COMMENT] ? meta[META_COMMENT][META_COMMENT_VALUE] : null || '',
    );
  ...
  refreshEditor(force = false) {
    ...
    const scrollableElement = getScrollableElement(
      this.hot.view.wt.wtTable.TABLE,
    );
    ...
    const lastColWidth =
      this.hot.view.wt.wtTable.getStretchedColumnWidth(column);
    let cellTopOffset = cellOffset.top < 0 ? 0 : cellOffset.top;
    let cellLeftOffset = cellOffset.left;

    if (
      this.hot.view.wt.wtViewport.hasVerticalScroll() &&
      scrollableElement !== window
    ) {
      cellTopOffset -=
        this.hot.view.wt.wtOverlays.topOverlay.getScrollPosition();
    }

    if (
      this.hot.view.wt.wtViewport.hasHorizontalScroll() &&
      scrollableElement !== window
    ) {
      cellLeftOffset -=
        this.hot.view.wt.wtOverlays.leftOverlay.getScrollPosition();
    }
  ...
  onMouseDown(event) {
    ...
    if (!this.contextMenuEvent && !this.targetIsCommentTextArea(event)) {
      ...
      if (
        ...
        (this.range.from &&
          coordinates &&
          (this.range.from.row !== coordinates.row ||
  ...
  onMouseOver(event) {
    ...
    priv.cellBelowCursor = document.elementFromPoint(
      event.clientX,
      event.clientY,
    );
    ...
    if (
      ...
      hasClass(event.target, 'wtBorder') ||
      priv.cellBelowCursor !== event.target ||
      !this.editor
    ) {
      return;
    }
    ...
    if (this.targetIsCellWithComment(event)) {
      ...
      isChildOf(event.target, document) &&
      !this.targetIsCommentTextArea(event)
    ) {
  ...
  onEditorMouseUp(event) {
    ...
    if (
      currentWidth !== priv.tempEditorDimensions.width + 1 ||
  ...
  addToContextMenu(defaultOptions) {
    defaultOptions.items.push(
      ...
      },
      {
        ...
        name: () => {
          if (this.checkSelectionCommentsConsistency()) {
            return this.hot.getTranslatedPhrase(
        ...
        disabled() {
          return !(
            this.getSelectedLast() && !this.selection.isSelectedByCorner()
          );
        },
      },
      {
      ...
      },
      {
        ...
        name() {
          let label = this.getTranslatedPhrase(
            C.CONTEXTMENU_ITEMS_READ_ONLY_COMMENT,
          );
          const hasProperty = checkSelectionConsistency(
