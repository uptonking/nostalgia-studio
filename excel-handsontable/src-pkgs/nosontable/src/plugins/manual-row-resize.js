...
class ManualRowResize extends BasePlugin {
  ...
  setupGuidePosition() {
    ...
    const handleRightPosition =
      parseInt(this.handle.style.left, 10) + handleWidth;
    const maximumVisibleElementWidth = parseInt(
      this.hot.view.maximumVisibleElementWidth(0),
      10,
    );
  ...
  afterMouseDownTimeout() {
    ...
    const resize = (selectedRow, forceRender) => {
      const hookNewSize = this.hot.runHooks(
        'beforeRowResize',
        selectedRow,
        this.newSize,
        true,
  ...
  onMouseDown(event) {
    if (hasClass(event.target, 'manualRowResizer')) {
      ...
      if (this.autoresizeTimeout === null) {
        this.autoresizeTimeout = setTimeout(
          () => this.afterMouseDownTimeout(),
          500,
        );
   ...
   */
  bindEvents() {
    this.eventManager.addEventListener(this.hot.rootElement, 'mouseover', (e) =>
      this.onMouseOver(e),
    );
    this.eventManager.addEventListener(this.hot.rootElement, 'mousedown', (e) =>
      this.onMouseDown(e),
    );
    this.eventManager.addEventListener(window, 'mousemove', (e) =>
      this.onMouseMove(e),
    );
    this.eventManager.addEventListener(window, 'mouseup', () =>
      this.onMouseUp(),
    );
  }
  ...
  onModifyRowHeight(height, row) {
    if (this.enabled) {
      ...
      const autoRowHeightResult = autoRowSizePlugin
        ? autoRowSizePlugin.heights[row]
        : null;
      const physicalRow = this.hot.runHooks('modifyRow', row);
