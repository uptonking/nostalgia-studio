...
class ManualColumnMove extends BasePlugin {
  ...
  enablePlugin() {
    ...
    this.addHook(
      ...
      (event, coords, TD, blockCalculations) =>
        this.onBeforeOnCellMouseDown(event, coords, TD, blockCalculations),
    );
    this.addHook(
      'beforeOnCellMouseOver',
      (event, coords, TD, blockCalculations) =>
        this.onBeforeOnCellMouseOver(event, coords, TD, blockCalculations),
    );
    ...
    this.addHook('beforeRemoveCol', (index, amount) =>
      this.onBeforeRemoveCol(index, amount),
    );
    this.addHook('afterRemoveCol', () => this.onAfterRemoveCol());
    this.addHook('afterCreateCol', (index, amount) =>
      this.onAfterCreateCol(index, amount),
    );
  ...
  moveColumns(columns, target) {
    ...
    const beforeColumnHook = this.hot.runHooks(
      'beforeColumnMove',
      visualColumns,
      target,
    );
  ...
  refreshPositions() {
    ...
    const scrollLeft =
      typeof scrollableElement.scrollX === 'number'
        ? scrollableElement.scrollX
        : scrollableElement.scrollLeft;
    let tdOffsetLeft =
      this.hot.view.THEAD.offsetLeft +
      this.getColumnsWidth(0, priv.coordsColumn);
    ...
    if (
      priv.rootElementOffset + wtTable.holder.offsetWidth + scrollLeft <
      priv.target.eventPageX
    ) {
    ...
    if (priv.hasRowHeaders) {
      rowHeaderWidth =
        this.hot.view.wt.wtOverlays.leftOverlay.clone.wtTable.getColumnHeader(
          -1,
        ).offsetWidth;
    }
    ...
    if (priv.coordsColumn < 0) {
      ...
      priv.target.TD.offsetWidth / 2 + tdOffsetLeft <=
      mouseOffsetLeft
    ) {
      const newCoordsCol =
        priv.coordsColumn >= priv.countCols
          ? priv.countCols - 1
          : priv.coordsColumn;
    ...
    if (
      mouseOffsetLeft + backlightElemWidth + backlightElemMarginLeft >=
      hiderWidth
    ) {
      ...
      mouseOffsetLeft + backlightElemMarginLeft <
      tbodyOffsetLeft + rowHeaderWidth
    ) {
    ...
    if (tdOffsetLeft >= hiderWidth - 1) {
      ...
      guidelineLeft -=
        priv.rootElementOffset <= scrollableElement.scrollX
          ? priv.rootElementOffset
          : 0;
    }
  ...
  updateColumnsMapper() {
    ...
    if (columnsMapperLen === 0) {
      this.columnsMapper.createMap(
        countCols || this.hot.getSettings().startCols,
      );
  ...
  onBeforeOnCellMouseDown(event, coords, TD, blockCalculations) {
    ...
    const isSortingElement =
      event.realTarget.className.indexOf('sortAction') > -1;
    ...
    const guidelineIsNotReady =
      this.guideline.isBuilt() && !this.guideline.isAppended();
    ...
    if (coords.row < 0 && coords.col >= start && coords.col <= end) {
      ...
      const topPos =
        wtTable.holder.scrollTop + wtTable.getColumnHeaderHeight(0) + 1;
      const fixedColumns = coords.col < priv.fixedColumns;
      const scrollableElement = this.hot.view.wt.wtOverlays.scrollableElement;
      const wrapperIsWindow = scrollableElement.scrollX
        ? scrollableElement.scrollX - priv.rootElementOffset
        : 0;

      const mouseOffset = event.layerX - (fixedColumns ? wrapperIsWindow : 0);
      const leftOffset = Math.abs(
        this.getColumnsWidth(start, coords.col) + mouseOffset,
      );

      this.backlight.setPosition(
        topPos,
        this.getColumnsWidth(countColumnsFrom, start) + leftOffset,
      );
      this.backlight.setSize(
        this.getColumnsWidth(start, end + 1),
  ...
  onMouseUp() {
    ...
    if (!priv.disallowMoving) {
      const selectionStart = this.columnsMapper.getIndexByValue(
        priv.columnsToMove[0],
      );
      const selectionEnd = this.columnsMapper.getIndexByValue(
        priv.columnsToMove[priv.columnsToMove.length - 1],
      );
  ...
  onBeforeRemoveCol(index, amount) {
    ...
    if (index !== false) {
      ...
      rangeEach(index, index + amount - 1, (removedIndex) => {
        this.removedColumns.push(
          this.hot.runHooks('modifyCol', removedIndex, this.pluginName),
        );
      });
    }
  }
