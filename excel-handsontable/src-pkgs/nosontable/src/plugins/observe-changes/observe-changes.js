...
class ObserveChanges extends BasePlugin {
  ...
  enablePlugin() {
    ...
    this.observer.addLocalHook('change', (patches) =>
      this.onDataChange(patches),
    );
    ...
    if (!this.observer.isPaused()) {
      ...
      const actions = {
        ...
        replace: (patch) => {
          this.hot.runHooks(
            'afterChange',
            [[patch.row, patch.col, null, patch.value]],
            sourceName,
          );
        },
      };
  ...
  onAfterTableAlter(source) {
    if (source !== 'loadData') {
      this.observer.pause();
      this.hot.addHookOnce('afterChangesObserved', () =>
