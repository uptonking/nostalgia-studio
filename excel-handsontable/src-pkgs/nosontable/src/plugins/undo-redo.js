...
function UndoRedo(instance) {
  ...
  instance.addHook('beforeRemoveRow', (index, amount, logicRows, source) => {
    ...
    const removedData = deepClone(
      originalData.slice(physicalRowIndex, physicalRowIndex + amount),
    );
  ...
  instance.addHook('beforeRemoveCol', (index, amount, logicColumns, source) => {
    ...
    const originalData = plugin.instance.getSourceDataArray();
    const columnIndex =
      (plugin.instance.countCols() + index) % plugin.instance.countCols();
    const removedData = [];
    const headers = [];
    const indexes = [];
      ...
      rangeEach(amount - 1, (i) => {
        headers.push(
          instance.getSettings().colHeaders[
            instance.runHooks('modifyCol', columnIndex + i)
          ] || null,
        );
      });
    }

    const manualColumnMovePlugin =
      plugin.instance.getPlugin('manualColumnMove');
    const columnsMap = manualColumnMovePlugin.isEnabled()
      ? manualColumnMovePlugin.columnsMapper.__arrayMap
      : [];
  ...
  instance.addHook(
    ...
    (stateBefore, range, type, alignment) => {
      const action = new UndoRedo.CellAlignmentAction(
        stateBefore,
        range,
        type,
        alignment,
...
UndoRedo.ChangeAction.prototype.undo = function (instance, undoneCallback) {
  ...
  for (let i = 0, len = data.length; i < len; i++) {
    if (
      ...
      data[i][0] + 1 + instance.getSettings().minSpareRows ===
        instance.countRows() &&
      emptyRowsAtTheEnd === instance.getSettings().minSpareRows
    ) {
      instance.alter(
        'remove_row',
        parseInt(data[i][0] + 1, 10),
        instance.getSettings().minSpareRows,
      );
    ...
    if (
      ...
      data[i][1] + 1 + instance.getSettings().minSpareCols ===
        instance.countCols() &&
      emptyColsAtTheEnd === instance.getSettings().minSpareCols
    ) {
      instance.alter(
        'remove_col',
        parseInt(data[i][1] + 1, 10),
        instance.getSettings().minSpareCols,
      );
      instance.undoRedo.doneActions.pop();
    }
...
UndoRedo.CreateColumnAction.prototype.undo = function (
  instance,
  undoneCallback,
) {
...
UndoRedo.CreateColumnAction.prototype.redo = function (
  instance,
  redoneCallback,
) {
 ...
 * @private
 */
UndoRedo.RemoveColumnAction = function (
  index,
  indexes,
  data,
  headers,
...
UndoRedo.RemoveColumnAction.prototype.undo = function (
  instance,
  undoneCallback,
) {
  let row;
  ...
  if (typeof this.headers !== 'undefined') {
    rangeEach(sortedHeaders.length - 1, (j) => {
      instance
        .getSettings()
        .colHeaders.splice(ascendingIndexes[j], 0, sortedHeaders[j]);
    });
  }

  if (instance.getPlugin('manualColumnMove')) {
    instance.getPlugin('manualColumnMove').columnsMapper.__arrayMap =
...
UndoRedo.RemoveColumnAction.prototype.redo = function (
  instance,
  redoneCallback,
) {
...
UndoRedo.CellAlignmentAction.prototype.undo = function (
  instance,
  undoneCallback,
) {
    ...
    for (let row = from.row; row <= to.row; row += 1) {
      for (let col = from.col; col <= to.col; col += 1) {
        instance.setCellMeta(
          ...
          col,
          'className',
          this.stateBefore[row][col] || ' htLeft',
        );
      }
    }
  });
...
UndoRedo.CellAlignmentAction.prototype.redo = function (
  instance,
  undoneCallback,
) {
  align(
    ...
    this.type,
    this.alignment,
    (row, col) => instance.getCellMeta(row, col),
...
UndoRedo.FiltersAction.prototype.undo = function (instance, undoneCallback) {
  ...
  filters.conditionCollection.importAllConditions(
...
UndoRedo.RowMoveAction.prototype.undo = function (instance, undoneCallback) {
  ...
  const newTarget =
    this.rows[0] > this.target ? this.rows[0] + this.rows.length : this.rows[0];
  const newRows = [];
...
UndoRedo.RowMoveAction.prototype.redo = function (instance, redoneCallback) {
  ...
  const startSelection =
    this.rows[0] < this.target ? this.target - this.rows.length : this.target;

  instance.selectCell(
    startSelection,
    0,
    startSelection + this.rows.length - 1,
    instance.countCols() - 1,
    false,
    false,
  );
...
function init() {
  const instance = this;
  const pluginEnabled =
    typeof instance.getSettings().undo === 'undefined' ||
    instance.getSettings().undo;
