...
class CopyPaste extends BasePlugin {
  ...
  setCopyableText() {
    ...
    this.copyableRanges = this.hot.runHooks(
      'modifyCopyableRange',
      this.copyableRanges,
    );
  ...
  populateValues(inputArray, selection = this.hot.getSelectedLast()) {
    ...
    const endRow = Math.max(
      selection[0],
      selection[2],
      newValuesMaxRow + startRow,
    );
    const startColumn = Math.min(selection[1], selection[3]);
    const endColumn = Math.max(
      selection[1],
      selection[3],
      newValuesMaxColumn + startColumn,
    );
    const newValues = [];
    ...
    for (let row = startRow, valuesRow = 0; row <= endRow; row += 1) {
      ...
      for (
        let column = startColumn, valuesColumn = 0;
        column <= endColumn;
        column += 1
      ) {
        ...
        valuesColumn =
          valuesColumn === newValuesMaxColumn ? 0 : (valuesColumn += 1);
      }
  ...
  onCopy(event) {
    ...
    if (
      (!this.hot.isListening() && !priv.isTriggeredByCopy) ||
      this.isEditorOpened()
    ) {
      return;
    }
    ...
    const allowCopying = !!this.hot.runHooks(
      'beforeCopy',
      rangedData,
      this.copyableRanges,
    );
  ...
  onCut(event) {
    ...
    if (
      (!this.hot.isListening() && !priv.isTriggeredByCut) ||
      this.isEditorOpened()
    ) {
      return;
    }
    ...
    const rangedData = this.getRangedData(this.copyableRanges);
    const allowCuttingOut = !!this.hot.runHooks(
      'beforeCut',
      rangedData,
      this.copyableRanges,
    );
  ...
  onPaste(event) {
    ...
    const inputArray =
      typeof pastedData !== 'string' ? pastedData : SheetClip.parse(pastedData);
    ...
    if (
      this.hot.runHooks('beforePaste', inputArray, this.copyableRanges) ===
      false
    ) {
      return;
    }
  ...
  onAfterSelectionEnd() {
    ...
    if (
      ...
      this.focusableElement.getFocusableElement() !== document.activeElement &&
      getSelectionText()
    ) {
      return;
    }
