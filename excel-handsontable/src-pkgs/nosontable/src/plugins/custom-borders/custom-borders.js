...
class CustomBorders extends BasePlugin {
  ...
  insertBorderIntoSettings(border, place) {
    ...
    const hasCustomSelections = this.checkCustomSelections(
      border,
      cellRange,
      place,
    );
  ...
  setBorder(row, column, place, remove) {
    ...
    if (remove) {
      ...
      const customSelectionsChecker = this.checkCustomSelectionsFromContextMenu(
        bordersMeta,
        place,
        remove,
      );
  ...
  spliceBorder(borderId) {
    const index = arrayMap(this.savedBorders, (border) => border.id).indexOf(
      borderId,
    );
