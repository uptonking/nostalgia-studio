...
const inheritedColumnProperties = [
  'sortEmptyCells',
  'indicator',
  'headerAction',
...
export class ColumnStatesManager {
  ...
  getSortOrderOfColumn(searchedColumn) {
    const searchedState = this.sortedColumnsStates.find(
      ({ column }) => searchedColumn === column,
    );
    let sortOrder;
  ...
  getColumnSortState(column) {
    if (this.isColumnSorted(column)) {
      return deepClone(
        this.sortedColumnsStates[this.getIndexOfColumnInSortQueue(column)],
      );
    }
  }
