...
import { registerRootComparator, sort } from './sort-service';
import {
  areValidSortStates,
...
class ColumnSorting extends BasePlugin {
  ...
  enablePlugin() {
    ...
    this.addHook('unmodifyRow', (row, source) =>
      this.onUnmodifyRow(row, source),
    );
    ...
    this.addHook('afterOnCellMouseDown', (event, target) =>
      this.onAfterOnCellMouseDown(event, target),
    );
    this.addHook('afterCreateRow', (index, amount) =>
      this.onAfterCreateRow(index, amount),
    );
    this.addHook('afterRemoveRow', (index, amount) =>
      this.onAfterRemoveRow(index, amount),
    );
    this.addHook('afterInit', () => this.loadOrSortBySettings());
    this.addHook('afterLoadData', (initialLoad) =>
      this.onAfterLoadData(initialLoad),
    );
  ...
  disablePlugin() {
    const clearColHeader = (column, TH) => {
      ...
      if (
        isFirstLevelColumnHeader(column, TH) === false ||
        headerSpanElement === null
      ) {
        return;
      }
  ...
  sort(sortConfig) {
    ...
    const allowSort = this.hot.runHooks(
      'beforeColumnSort',
      currentSortConfig,
    ...
    if (sortPossible) {
      const translateColumnToPhysical = ({
        column: visualColumn,
        ...restOfProperties
      }) => ({
      ...
      const internalSortStates = arrayMap(
        destinationSortConfigs,
  ...
  isSorted() {
    return (
      this.enabled && !this.columnStatesManager.isListOfSortedColumnsEmpty()
    );
  }
  ...
  getSortConfig(column) {
    ...
    if (isDefined(column)) {
      ...
      const columnSortState =
        this.columnStatesManager.getColumnSortState(physicalColumn);
    ...
    return arrayMap(sortStates, (columnState) =>
      translateColumnToVisual(columnState),
    );
  }
  ...
  areValidSortConfigs(sortConfigs) {
    ...
    const onlyExistingVisualIndexes = sortedColumns.every(
      (visualColumn) => visualColumn <= numberOfColumns && visualColumn >= 0,
    );
  ...
  getAllSavedSortSettings() {
    ...
    if (
      isDefined(allSortSettings) &&
      Array.isArray(allSortSettings.initialConfig)
    ) {
      allSortSettings.initialConfig = arrayMap(
  ...
  getNextSortConfig(
    ...
    const indexOfColumnToChange =
      this.columnStatesManager.getIndexOfColumnInSortQueue(physicalColumn);
    const isColumnSorted =
      this.columnStatesManager.isColumnSorted(physicalColumn);
    const currentSortConfig = this.getSortConfig();
  ...
  setMergedPluginSettings(column) {
    ...
    const columnMetaHasPluginSettings = Object.hasOwnProperty.call(
      columnMeta,
      this.pluginKey,
    );
    const pluginColumnConfig = columnMetaHasPluginSettings
      ? columnMeta[this.pluginKey]
      : {};

    this.columnMetaCache.set(
      physicalColumnIndex,
      Object.assign(
        storedColumnProperties,
        pluginMainSettings,
        pluginColumnConfig,
      ),
    );
  ...
  getFirstCellSettings(column) {
    ...
    if (
      this.columnMetaCache.size === 0 ||
      this.columnMetaCache.size < this.hot.countCols()
    ) {
    ...
    cellMetaCopy[this.pluginKey] = this.columnMetaCache.get(
  ...
  rebuildColumnMetaCache() {
    ...
    if (numberOfColumns === 0) {
      ...
      rangeEach(numberOfColumns - 1, (visualColumnIndex) =>
        this.setMergedPluginSettings(visualColumnIndex),
      );
    }
  }
  ...
  sortByPresetSortStates() {
    ...
    for (
      ...
      visualRowIndex < this.getNumberOfRowsToSort(numberOfRows);
      visualRowIndex += 1
    ) {
      indexesWithData.push(
        [visualRowIndex].concat(getDataForSortedColumns(visualRowIndex)),
      );
    }
    ...
    sort(
      ...
      this.pluginKey,
      arrayMap(sortedColumnsList, (physicalColumn) =>
        this.columnStatesManager.getSortOrderOfColumn(physicalColumn),
      ),
      arrayMap(sortedColumnsList, (physicalColumn) =>
        this.getFirstCellSettings(this.hot.toVisualColumn(physicalColumn)),
      ),
    );

    // Append spareRows
    for (
      let visualRowIndex = indexesWithData.length;
      visualRowIndex < numberOfRows;
      visualRowIndex += 1
    ) {
      indexesWithData.push(
        [visualRowIndex].concat(getDataForSortedColumns(visualRowIndex)),
    ...
    this.rowsMapper._arrayMap = arrayMap(
      indexesWithData,
  ...
  onBeforeOnCellMouseDown(event, coords, TD, blockCalculations) {
    ...
    if (
      this.wasClickableHeaderClicked(event, coords.col) &&
      isPressedCtrlKey()
    ) {
