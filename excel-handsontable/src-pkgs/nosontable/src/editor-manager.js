...
import { EditorState } from './editors/base-editor';
...
function EditorManager(instance, priv, selection) {
  ...
  function moveSelectionAfterEnter(shiftKey) {
    const enterMoves =
      typeof priv.settings.enterMoves === 'function'
        ? priv.settings.enterMoves(event)
        : priv.settings.enterMoves;
  ...
  function onKeyDown(event) {
    ...
    switch (event.keyCode) {
      ...
      case KEY_CODES.TAB:
        tabMoves =
          typeof priv.settings.tabMoves === 'function'
            ? priv.settings.tabMoves(event)
            : priv.settings.tabMoves;
      ...
      case KEY_CODES.HOME:
        if (event.ctrlKey || event.metaKey) {
          rangeModifier.call(
            selection,
            new CellCoords(0, selection.selectedRange.current().from.col),
          );
        } else {
          rangeModifier.call(
            selection,
            new CellCoords(selection.selectedRange.current().from.row, 0),
          );
        }
  ...
  this.prepareEditor = function () {
    ...
    const originalValue = instance.getSourceDataAtCell(
      instance.runHooks('modifyRow', row),
      col,
    );
