import { arrayMap, arrayMax, arrayReduce } from '../helpers/array';
...
const arrayMapper = {
  ...
  getIndexByValue(translatedIndex) {
    ...
    return (physicalIndex = this._arrayMap.indexOf(translatedIndex)) === -1
      ? null
      : physicalIndex;
  },
  ...
  insertItems(physicalIndex, amount = 1) {
    ...
    rangeEach(amount - 1, (count) => {
      addedItems.push(
        this._arrayMap.splice(physicalIndex + count, 0, newIndex + count),
      );
    });
  ...
  swapIndexes(physicalIndexFrom, physicalIndexTo) {
    this._arrayMap.splice(
      physicalIndexTo,
      0,
      ...this._arrayMap.splice(physicalIndexFrom, 1),
    );
  },
