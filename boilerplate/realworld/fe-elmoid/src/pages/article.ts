...
import { editor, HOME, LOGIN, profile, REGISTER } from './links';

// import { ChangeFieldFromTarget, LogError } from './fragments/forms';
...
const ArticleMeta = ({ state }) => html`
  <div class="article-meta">
    ...
    <div class="info">
      <a
        data-test="author"
        href=${profile(state.author.username)}
        class="author"
      >
...
const ArticleBanner = ({ state }) => html`
  <div class="banner">
    <div class="container">
...
const Comment = ({ comment, slug, user }) => html`
...
const CommentList = ({ comments, user, slug }) => {
  return html`
    <div>${comments.map((comment) => Comment({ comment, slug, user }))}</div>
  `;
};
...
const CommentContainer = ({ state }) => html`
  <div class="col-xs-12 col-md-8 offset-md-2">
    ${state.user.token
      ? html` <div>${CommentInput({ state })}</div> `
      : html`
    ...
    ? html`
        <div class="article-page">
          ...
          <div class="container page">
            ...
            <div class="row">${CommentContainer({ state })}</div>
          </div>
        </div>
      `
    : '';
