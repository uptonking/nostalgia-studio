...
const FetchFeed = ({
  ...
  return backendFeeds[activeFeedType]({
...
const GlobalFeed = ({ activeFeedType }) =>
  FeedTab(
    // @ts-expect-error fix-types
    { active: activeFeedType === GLOBAL_FEED, type: GLOBAL_FEED },
    'For You',
  );
...
const FeedTab = ({ active, type, name }, children) => html`
  <li class="nav-item">
    <a
      href=""
      data-test="feed"
      ...
      onclick=${[
        // preventDefault(ChangeTab),
        eventWith({ activeFeedName: name, activeFeedType: type }),
      ]}
    >
      ${children}
    </a>
    ...
    ${tags.map((tag) => {
      return html`
        <a
          ...
          onclick=${[
            // preventDefault(ChangeTab),
            eventWith({ activeFeedType: TAG_FEED, activeFeedName: tag }),
          ]}
        >
          ${tag}
        </a>
          ...
          (page) => html`
            <li class=${page.isCurrent ? 'page-item active' : 'page-item'}>
              <a
                ...
                href=""
                onclick=${[
                  // preventDefault(ChangePage),
                  eventWith({ currentPageIndex: page.index }),
                ]}
              >
                ${page.humanDisplay}
              </a>
        ...
        <div class="col-md-9">
          <div class="feed-toggle">
            <ul data-test="feeds" class="nav nav-pills outline-active">
              ${feeds.map((name) =>
                uiFeeds[name]({ activeFeedType, activeFeedName }),
              )}
            </ul>
          </div>
          ${ArticleList(
