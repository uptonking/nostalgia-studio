...
export class ChangeSet<Data = any> {
  ...
  map(f: (range: Span<Data>) => Data): ChangeSet<Data> {
    ...
    return new ChangeSet(
      this.config,
      this.changes.map((ch: Change<Data>) => {
        return new Change(
          ch.fromA,
          ch.toA,
          ch.fromB,
  ...
  changedRange(
    ...
    let to = touched ? touched.toB : -2e8;
  ...
  static create<Data = any>(
    doc: Node,
    combine: (dataA: Data, dataB: Data) => Data = (a, b) =>
      a === b ? a : (null as any),
...
function mergeAll<Data>(
  ...
  let mid = (start + end) >> 1;
  return Change.merge(
    mergeAll(ranges, combine, start, mid),
    mergeAll(ranges, combine, mid, end),
    combine,
  );
}
