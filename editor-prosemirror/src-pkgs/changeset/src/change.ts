...
export class Span<Data = any> {
  ...
  /// @internal
  static join<Data>(
    a: readonly Span<Data>[],
    b: readonly Span<Data>[],
...
export class Change<Data = any> {
  ...
  static merge<Data>(
    ...
    for (
      let iX = 0,
        iY = 0,
        curX: Change<Data> | null = x[0],
        curY: Change<Data> | null = y[0];
      ;
      ...
      if (!curX && !curY) {
        ...
        let fromA = Math.min(
          curX!.fromA,
          curY!.fromA - (iX ? x[iX - 1].toB - x[iX - 1].toA : 0),
        );
        let toA = fromA;
        let fromB = Math.min(
          curY!.fromB,
          curX!.fromB + (iY ? y[iY - 1].toB - y[iY - 1].toA : 0),
        );
        let toB = fromB;
        ...
        for (;;) {
          ...
          if (inX && !inY) {
            inserted = Span.join(
              inserted,
              Span.slice(curX!.inserted, pos - curX!.fromB, next - curX!.fromB),
              combine,
            );
            toB += next - pos;
          }
          ...
          if (inY && !inX) {
            deleted = Span.join(
              deleted,
              Span.slice(curY!.deleted, pos - curY!.fromA, next - curY!.fromA),
              combine,
            );
            toA += next - pos;
          }
