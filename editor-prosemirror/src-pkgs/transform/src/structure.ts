...
export function liftTarget(range: NodeRange): number | null {
  ...
  for (
    let depth = range.depth, contentBefore = 0, contentAfter = 0;
    ;
    --depth
  ) {
...
export function setBlockType(
  ...
  to: number,
  type: NodeType,
  attrs: Attrs | null | ((oldNode: Node) => Attrs),
) {
  if (!type.isTextblock)
  ...
  tr.doc.nodesBetween(from, to, (node, pos) => {
    ...
    if (
      ...
      if (type.schema.linebreakReplacement) {
        ...
        let supportLinebreak = !!type.contentMatch.matchType(
      ...
      tr.step(
        new ReplaceAroundStep(
          ...
          endM - 1,
          new Slice(
            Fragment.from(type.create(attrsHere, null, node.marks)),
            0,
            0,
          ),
          1,
...
function canAppendWithSubstitutedLinebreaks(a: Node, b: Node) {
  ...
  for (let i = 0; i < b.childCount; i++) {
    let child = b.child(i);
    let type =
      child.type == linebreakReplacement
...
export function join(tr: Transform, pos: number, depth: number) {
  ...
  if (linebreakReplacement && beforeType.inlineContent) {
    ...
    let supportLinebreak =
      !!beforeType.contentMatch.matchType(linebreakReplacement);
    if (pre && !supportLinebreak) convertNewlines = false;
    else if (!pre && supportLinebreak) convertNewlines = true;
  }
  let mapFrom = tr.steps.length;
  ...
  tr.step(
    new ReplaceStep(start, mapping.map(pos + depth, -1), Slice.empty, true),
  );
