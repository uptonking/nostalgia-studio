    ...
    for (
      ...
      if (
        ...
        (rule.namespace === undefined ||
          (dom as HTMLElement).namespaceURI == rule.namespace) &&
...
class ParseContext {
  ...
  addElement(
    ...
    let ruleID: TagParseRule | undefined;
        ...
        if (value)
          for (let after: StyleParseRule | undefined = undefined; ; ) {
            ...
            if (rule.clearMark)
              ...
              marks = marks.concat(
                this.parser.schema.marks[rule.mark!].create(rule.attrs),
              );
  ...
  enterInner(
    ...
    marks = marks.filter((m) => {
      if (
        top.type ? top.type.allowsMarkType(m.type) : markMayApply(m.type, type)
      ) {
    ...
    this.nodes.push(
      new NodeContext(type, attrs, applyMarks, solid, null, options),
    );
    this.open++;
    return marks;
  }
