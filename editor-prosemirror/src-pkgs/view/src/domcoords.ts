...
export function scrollRectIntoView(
  ...
  for (let parent: Node | null = startDOM || view.dom; ; ) {
    ...
    let pos: string = atTop
      ? 'fixed'
      : getComputedStyle(parent as HTMLElement).position;
    if (/^(fixed|sticky)$/.test(pos)) break;
    parent =
      pos == 'absolute'
        ? (parent as HTMLElement).offsetParent
        : parentNode(parent);
  }
}
...
function findOffsetInNode(
  ...
  let coordsBelow: { left: number; top: number } | undefined;
  for (
...
function posFromCaret(
  ...
  for (let cur = node, sawBlock = false; ; ) {
    ...
    if (
      ...
      ((rect = (desc.dom as HTMLElement).getBoundingClientRect()).width ||
        rect.height)
    ) {
      if (
        desc.node.isBlock &&
        ...
        if ((!sawBlock && rect.left > coords.left) || rect.top > coords.top)
          ...
          (!sawBlock && rect.right < coords.left) ||
