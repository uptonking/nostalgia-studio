...
export class DecorationSet implements DecorationSource {
  ...
  private findInner(
    ...
    for (let i = 0; i < this.children.length; i += 3) {
      if (
        (this.children[i] as number) < end &&
        (this.children[i + 1] as number) > start
      ) {
  ...
  private addInner(doc: Node, decorations: Decoration[], offset: number) {
    ...
    doc.forEach((childNode, childOffset) => {
      ...
      while (
        ...
        (children[childIndex] as number) < childOffset
      )
        childIndex += 3;
      if (children[childIndex] == childOffset)
        children[childIndex + 2] = (
          children[childIndex + 2] as DecorationSet
  ...
  private removeInner(decorations: (Decoration | null)[], offset: number) {
    ...
    for (let i = 0; i < children.length; i += 3) {
      ...
      let to = (children[i + 1] as number) + offset;
  ...
  forChild(offset: number, node: Node): DecorationSet | DecorationGroup {
    ...
    for (let i = 0; i < this.children.length; i += 3)
      if ((this.children[i] as number) >= offset) {
  ...
  eq(other: DecorationSet) {
    ...
    for (let i = 0; i < this.children.length; i += 3)
      if (
        ...
        !(this.children[i + 2] as DecorationSet).eq(
          other.children[i + 2] as DecorationSet,
        )
      )
        return false;
    return true;
...
function takeSpansForNode(
  spans: (Decoration | null)[],
  node: Node,
  offset: number,
): Decoration[] | null {
