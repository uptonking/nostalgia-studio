...
function selectHorizontally(view: EditorView, dir: number, mods: string) {
  ...
  if (sel instanceof TextSelection) {
    if (mods.indexOf('s') > -1) {
      let { $head } = sel;
      let node = $head.textOffset
        ? null
        : dir < 0
          ? $head.nodeBefore
          : $head.nodeAfter;
      if (!node || node.isText || !node.isLeaf) return false;
      let $newHead = view.state.doc.resolve(
        $head.pos + node.nodeSize * (dir < 0 ? -1 : 1),
      );
      ...
      if (NodeSelection.isSelectable(node)) {
        return apply(
          view,
          new NodeSelection(
            dir < 0 ? view.state.doc.resolve($head.pos - node.nodeSize) : $head,
          ),
        );
        ...
        return apply(
          view,
          new TextSelection(
            view.state.doc.resolve(dir < 0 ? nodePos : nodePos + node.nodeSize),
          ),
        );
      } else {
        return false;
...
function isIgnorable(dom: Node, dir: number) {
  ...
  return (
    desc &&
    desc.size == 0 &&
    (dir < 0 || dom.nextSibling || dom.nodeName != 'BR')
  );
}
...
function textNodeAfter(node: Node | null, offset: number): Text | undefined {
  ...
  while (node && offset < node.childNodes.length) {
    ...
    if (next.nodeType == 1 && (next as HTMLElement).contentEditable == 'false')
      break;
    node = next;
    offset = 0;
  }
...
export function captureKeyDown(view: EditorView, event: KeyboardEvent) {
  ...
  if (code == 8 || (browser.mac && code == 72 && mods == 'c')) {
    ...
    (code == 46 && !event.shiftKey) ||
    (browser.mac && code == 68 && mods == 'c')
  ) {
    ...
    let dir =
      code == 37
        ? findDirection(view, view.state.selection.from) == 'ltr'
          ? -1
          : 1
        : -1;
