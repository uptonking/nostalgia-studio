...
class CompletionTooltip {
    ...
    this.dom.addEventListener('mousedown', (e: MouseEvent) => {
      ...
      if (e.target == this.list) {
        let move =
          ...
          e.clientY <
            (this.list.firstChild as HTMLElement).getBoundingClientRect().top
            ? this.range.from - 1
            : this.list.classList.contains(
                  'cm-completionListIncompleteBottom',
                ) &&
                e.clientY >
                  (this.list.lastChild as HTMLElement).getBoundingClientRect()
                    .bottom
              ? this.range.to
              : null;
        if (move != null) {
  ...
  addInfoPane(content: NonNullable<CompletionInfo>, completion: Completion) {
    ...
    wrap.id =
      'cm-completionInfo-' + Math.floor(Math.random() * 0xffff).toString(16);
