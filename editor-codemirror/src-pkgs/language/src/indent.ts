...
export const indentService =
  Facet.define<
    (context: IndentContext, pos: number) => number | null | undefined
  >();
...
export function getIndentation(
  context: IndentContext | EditorState,
  pos: number,
): number | null {
...
export const indentNodeProp = new NodeProp<
  (context: TreeIndentContext) => number | null
>();
...
function indentStrategy(
  ...
  if (first && (close = first.type.prop(NodeProp.closedBy))) {
    ...
    let closed = last && close.indexOf(last.name) > -1;
    return (cx) =>
      delimitedStrategy(
        cx,
        true,
...
/// `units` option.
export function continuedIndent({
  except,
  units = 1,
