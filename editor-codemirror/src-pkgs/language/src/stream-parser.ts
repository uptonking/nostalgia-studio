...
export class StreamLanguage<State> extends Language {
  ...
  getIndent(cx: TreeIndentContext) {
    ...
    while (statePos < cx.pos) {
      ...
      let end = Math.min(cx.pos, line.to);
      if (line.length) {
...
function createTokenType(
  ...
  let key = name + ' ' + tags.map((t) => (t as any).id);
  let known = byTag[key];
