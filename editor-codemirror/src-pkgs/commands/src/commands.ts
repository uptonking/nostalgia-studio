...
export const cursorCharLeft: Command = (view) =>
  cursorByChar(view, !ltrAtCursor(view));
/// Move the selection one character to the right.
export const cursorCharRight: Command = (view) =>
  cursorByChar(view, ltrAtCursor(view));
...
function moveBySubword(
  ...
  if (
    segmenter &&
    cat == (CharCategory.Word as any) &&
    end.from == range.from + steps * (forward ? 1 : -1)
  ) {
    let from = Math.min(range.head, end.head);
    let to = Math.max(range.head, end.head);
    let skipped = view.state.sliceDoc(from, to);
    if (skipped.length > 1 && /[\u4E00-\uffff]/.test(skipped)) {
      let segments = Array.from(segmenter.segment(skipped)) as {
        index: number;
      }[];
...
export const cursorSubwordForward: Command = (view) =>
  cursorBySubword(view, true);
/// Move the selection one group or camel-case subword backward.
export const cursorSubwordBackward: Command = (view) =>
  cursorBySubword(view, false);

function interestingNode(
  state: EditorState,
  node: SyntaxNode,
  bracketProp: NodeProp<unknown>,
) {
...
export const cursorSyntaxLeft: Command = (view) =>
  moveSel(view, (range) => moveBySyntax(view.state, range, !ltrAtCursor(view)));
/// Move the cursor over the next syntactic element to the right.
export const cursorSyntaxRight: Command = (view) =>
  moveSel(view, (range) => moveBySyntax(view.state, range, ltrAtCursor(view)));
...
function cursorByPage(view: EditorView, forward: boolean) {
  ...
  if (page.selfScroll) {
    ...
    let scrollBottom = scrollRect.bottom - page.marginBottom;
...
/// on, if any.
export const cursorMatchingBracket: StateCommand = ({ state, dispatch }) =>
  toMatchingBracket(state, dispatch, false);
/// Extend the selection to the bracket matching the one the selection
/// head is currently on, if any.
export const selectMatchingBracket: StateCommand = ({ state, dispatch }) =>
...
/// the anchor in place.
export const selectCharLeft: Command = (view) =>
  selectByChar(view, !ltrAtCursor(view));
/// Move the selection head one character to the right.
export const selectCharRight: Command = (view) =>
  selectByChar(view, ltrAtCursor(view));
...
/// the left.
export const selectGroupLeft: Command = (view) =>
  selectByGroup(view, !ltrAtCursor(view));
/// Move the selection head one group to the right.
export const selectGroupRight: Command = (view) =>
  selectByGroup(view, ltrAtCursor(view));
...
export const selectSubwordForward: Command = (view) =>
  selectBySubword(view, true);
/// Move the selection head one group or subword backward.
export const selectSubwordBackward: Command = (view) =>
  selectBySubword(view, false);

/// Move the selection head over the next syntactic element to the left.
export const selectSyntaxLeft: Command = (view) =>
  extendSel(view, (range) =>
    moveBySyntax(view.state, range, !ltrAtCursor(view)),
  );
/// Move the selection head over the next syntactic element to the right.
export const selectSyntaxRight: Command = (view) =>
  extendSel(view, (range) =>
    moveBySyntax(view.state, range, ltrAtCursor(view)),
  );
...
export const selectLineBoundaryForward: Command = (view) =>
  extendSel(view, (range) => moveByLineBoundary(view, range, true));
/// Move the selection head to the previous line boundary.
export const selectLineBoundaryBackward: Command = (view) =>
  extendSel(view, (range) => moveByLineBoundary(view, range, false));
/// Move the selection head one line boundary to the left.
export const selectLineBoundaryLeft: Command = (view) =>
  extendSel(view, (range) =>
    moveByLineBoundary(view, range, !ltrAtCursor(view)),
  );
/// Move the selection head one line boundary to the right.
export const selectLineBoundaryRight: Command = (view) =>
  extendSel(view, (range) =>
    moveByLineBoundary(view, range, ltrAtCursor(view)),
  );

/// Move the selection head to the start of the line.
export const selectLineStart: Command = (view) =>
  extendSel(view, (range) =>
    EditorSelection.cursor(view.lineBlockAt(range.head).from),
  );
/// Move the selection head to the end of the line.
export const selectLineEnd: Command = (view) =>
  extendSel(view, (range) =>
...
function addCursorVertically(view: EditorView, forward: boolean) {
  ...
  let ranges: SelectionRange[] = state.selection.ranges.slice();
  ...
  view.dispatch(
    setSel(state, EditorSelection.create(ranges, ranges.length - 1)),
  );
  return true;
}
...
export const addCursorAbove: Command = (view) =>
  addCursorVertically(view, false);

/// Expand the selection by adding a cursor below the heads of
/// currently selected ranges.
export const addCursorBelow: Command = (view) =>
  addCursorVertically(view, true);
...
function deleteBy(
  target: CommandTarget,
  by: (start: SelectionRange) => number,
) {
...
const deleteByChar = (
  ...
  deleteBy(target, (range) => {
    ...
    if (
      ...
      let drop = col % getIndentUnit(state) || getIndentUnit(state);
...
export const deleteCharBackward: Command = (view) =>
  deleteByChar(view, false, true);

/// Delete the selection or the character before the cursor. Does not
/// implement any extended behavior like deleting whole indentation
/// units in one go.
export const deleteCharBackwardStrict: Command = (view) =>
  deleteByChar(view, false, false);

/// Delete the selection or the character after the cursor.
export const deleteCharForward: Command = (view) =>
  deleteByChar(view, true, false);
...
export const deleteGroupBackward: StateCommand = (target) =>
  deleteByGroup(target, false);
/// Delete the selection or forward until the end of the next group.
export const deleteGroupForward: StateCommand = (target) =>
...
/// after a word.
export const deleteGroupForwardWin: Command = (view) =>
  deleteBy(
    view,
    (range) =>
      view.moveByChar(range, true, (start) =>
...
export const deleteToLineEnd: Command = (view) =>
  deleteBy(view, (range) => {
...
export const deleteLineBoundaryBackward: Command = (view) =>
  deleteBy(view, (range) => {
...
export const deleteLineBoundaryForward: Command = (view) =>
  deleteBy(view, (range) => {
...
function selectedLineBlocks(state: EditorState) {
  let blocks: { from: number; to: number; ranges: SelectionRange[] }[] = [];
  let upto = -1;
  for (let range of state.selection.ranges) {
...
export const moveLineUp: StateCommand = ({ state, dispatch }) =>
  moveLine(state, dispatch, false);
/// Move the selected lines down one line.
export const moveLineDown: StateCommand = ({ state, dispatch }) =>
...
export const copyLineUp: StateCommand = ({ state, dispatch }) =>
  copyLine(state, dispatch, false);
/// Create a copy of the selected lines. Keep the selection in the bottom copy.
export const copyLineDown: StateCommand = ({ state, dispatch }) =>
...
function isBetweenBrackets(
  ...
  let closedBy: readonly string[] | undefined;
  if (
    before &&
    after &&
    before.to <= pos &&
...
export const standardKeymap: readonly KeyBinding[] = (
  ...
  emacsStyleKeymap.map((b) => ({ mac: b.key, run: b.run, shift: b.shift })),
);
