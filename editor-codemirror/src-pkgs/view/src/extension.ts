...
export const clickAddsSelectionRange =
  Facet.define<(event: MouseEvent) => boolean>();

export const dragMovesSelection =
  Facet.define<(event: MouseEvent) => boolean>();
...
export const scrollIntoView = StateEffect.define<ScrollTarget>({
  map: (t, ch) => t.map(ch),
});
...
export function getIsolatedRanges(
  view: EditorView,
  line: Line,
): readonly Isolate[] {
...
export class ViewUpdate {
  ...
  get geometryChanged() {
    return (
      this.docChanged ||
      (this.flags & (UpdateFlag.Geometry | UpdateFlag.Height)) > 0
    );
  }
