...
function rectanglesForRange(
  ...
  let lineStyle = lineElt && window.getComputedStyle(lineElt);
  let leftSide =
    contentRect.left +
    (lineStyle
  ...
  let endBlock = blockAt(view, to, -1);
      ...
      if (r.to > start && r.from < end) {
        for (
          ...
          for (let span of view.bidiSpans(docLine)) {
            ...
            let spanTo = span.to + docLine.from;
...
class LayerView {
  measureReq: {
    read: () => readonly LayerMarker[];
    write: (markers: readonly LayerMarker[]) => void;
  };
  dom: HTMLElement;
  ...
  setOrder(state: EditorState) {
    let pos = 0;
    let order = state.facet(layerOrder);
