...
export abstract class Tile {
  ...
  get breakAfter(): 0 | 1 {
    return (this.flags & TileFlag.BreakAfter) as 0 | 1;
  }
  ...
  isComposite(): this is CompositeTile {
    return false;
  }

  isLine(): this is LineTile {
    return false;
  }

  isText(): this is TextTile {
    return false;
  }
  ...
  markDirty(attrs: boolean) {
    ...
    if (this.parent && this.parent.flags & TileFlag.Synced)
  ...
  get root(): DocTile | null {
    for (let t: Tile | null = this; t; t = t.parent)
      if (t instanceof DocTile) return t;
    return null;
  }
...
export class DocTile extends CompositeTile {
  ...
  blockTiles<T>(
    f: (tile: WidgetTile | LineTile, pos: number) => T | undefined,
  ) {
    for (
      let stack: number[] = [], cur: CompositeTile = this, i = 0, pos = 0;
      ;
  ...
  resolveBlock(
    pos: number,
    side: number,
  ): { tile: LineTile | WidgetTile; offset: number } {
    let before: LineTile | WidgetTile | undefined;
    let beforeOff = -1;
    let after: LineTile | WidgetTile | undefined;
    let afterOff = -1;
    this.blockTiles((tile, off) => {
      ...
      if (pos >= off && pos <= end) {
        ...
        if (
          (off < pos ||
            (pos == end && (side < -1 ? tile.length : tile.covers(1)))) &&
        ...
        if (
          (end > pos ||
            (pos == off && (side > 1 ? tile.length : tile.covers(-1)))) &&
    ...
    return (before && side < 0) || !after
      ? { tile: before!, offset: beforeOff }
      : { tile: after, offset: afterOff };
  }
}
...
export class LineTile extends CompositeTile {
  ...
  resolveInline(
      ...
      for (let i = 0, off = 0; i < tile.children.length && off <= pos; i++) {
        ...
        if (end >= pos) {
          if (child.isComposite()) {
            ...
            (!after ||
              (after.isHidden &&
                (side > 0 || (forCoords && onSameLine(after, child))))) &&
            ...
            off < pos ||
            (child.flags & TileFlag.Before && !child.isHidden)
          ) {
            before = child;
            beforeOff = pos - off;
          }
    ...
    scan(this, pos);
    let target = ((side < 0 ? before : after) || before || after) as
      | TextTile
      | WidgetTile
      | WidgetBufferTile
      | null;
    return target
      ? { tile: target, offset: target == before ? beforeOff : afterOff }
      : null;
  }
    ...
    if (found) {
      ...
      if (this.dom.contains(tile.dom)) {
        if (tile.isText())
          return new DOMPos(
            tile.dom,
            Math.min(tile.dom.nodeValue!.length, offset),
          );
        return tile.domPosFor(
          offset,
          tile.flags & TileFlag.Before
            ? 1
            : tile.flags & TileFlag.After
              ? -1
              : side,
        );
      }
...
export class WidgetTile extends Tile {
  ...
  coordsInWidget(pos: number, side: number, block: boolean) {
    ...
    if (block) {
      ...
      if (!rects.length) return null;
      let fromBack =
        this.flags & TileFlag.Before
          ? true
          : this.flags & TileFlag.After
            ? false
            : pos > 0;
...
export class TilePointer {
  ...
  advance(dist: number, side: -1 | 1, walker?: TileWalker) {
    ...
    while (dist || side > 0) {
      if (!tile.isComposite()) {
        ...
        if (
          (side > 0 ? next.length <= dist : next.length < dist) &&
          (!walker ||
            walker.skip(next, 0, next.length) !== false ||
            !next.isComposite)
        ) {
          beforeBreak = !!brk;
          index++;
  ...
  get root() {
    return (this.parents.length ? this.parents[0].tile : this.tile) as DocTile;
  }
}
