...
export class DocView {
  ...
  update(update: ViewUpdate) {
    ...
    if (
      ...
      !changedRanges.some(
        (r) =>
          r.fromA <= composition!.range.fromA &&
          r.toA >= composition!.range.toA,
      )
    )
    ...
    observer.ignore(() => {
      ...
      if (
        ...
        (track.written ||
          ...
          !this.tile.dom.contains(track.node))
      )
        this.forceSelection = true;
      this.tile.dom.style.height = '';
    });
  ...
  updateSelection(mustRead = false, fromPointer = false) {
    ...
    let focused = activeElt == dom;
    let selectionNotFocus =
      !focused &&
      !(this.view.state.facet(editable) || dom.tabIndex > -1) &&
      hasSelection(dom, this.view.observer.selectionRange) &&
      !(activeElt && dom.contains(activeElt));
    ...
    if (main.empty) {
      ...
      anchor = this.inlineDOMNearPos(
        main.anchor,
        main.anchor == main.from ? 1 : -1,
      );
    }
    ...
    if (
      ...
      this.view.observer.ignore(() => {
        ...
        if (!rawSel) {
          ...
          if (
            main.bidiLevel != null &&
            (rawSel as any).caretBidiLevel !== undefined
          )
  ...
  enforceCursorAssoc() {
    ...
    let after = this.coordsAt(cursor.head, 1);
  ...
  posFromDOM(node: Node, offset: number): number {
    ...
    if (!tile)
      return this.tile.dom.compareDocumentPosition(node) & 2 /* PRECEDING */
        ? 0
    ...
    if (tile.isComposite()) {
      ...
      return node == tile.dom
        ? start + offset
        : start + (offset ? tile.length : 0);
    } else {
      return start;
    }
  }
  ...
  inlineDOMNearPos(pos: number, side: number): DOMPos {
    let before: LineTile | undefined | null;
    let beforeOff = -1;
    let beforeBad = false;
    let after: LineTile | undefined | null;
    let afterOff = -1;
    let afterBad = false;
    ...
    return (before && side < 0) || !after
      ? before!.domIn(beforeOff, side)
      : after.domIn(afterOff, side);
  }
  ...
  measureVisibleLineHeights(viewport: { from: number; to: number }) {
    ...
    let spaceAbove = 0;
    let scan = (
      tile: DocTile | BlockWrapperTile,
      pos: number,
      measureBounds: DOMRect | null,
    ) => {
      for (let i = 0; i < tile.children.length; i++) {
        if (pos > to) break;
        let child = tile.children[i];
        let end = pos + child.length;
        let childRect = (child.dom as HTMLElement).getBoundingClientRect();
        let { height } = childRect;
        if (measureBounds && !i)
  ...
  textDirectionAt(pos: number) {
    ...
    return getComputedStyle(tile.dom).direction == 'rtl'
      ? Direction.RTL
      : Direction.LTR;
  }
  ...
  updateDeco() {
    ...
    let outerDeco = this.view.state.facet(outerDecorations).map((d, i) => {
    ...
    this.blockWrappers = this.view.state
      .facet(blockWrappers)
      .map((v) => (typeof v == 'function' ? v(this.view) : v));
  }
  ...
  scrollIntoView(target: ScrollTarget) {
    ...
    let rect = this.coordsAt(
      range.head,
      range.assoc ?? (range.empty ? 0 : range.head > range.anchor ? -1 : 1),
    );
    let other;
    if (!rect) return;
    if (
...
function betweenUneditable(pos: DOMPos) {
  return (
    ...
    pos.node.firstChild &&
    (pos.offset == 0 ||
      (pos.node.childNodes[pos.offset - 1] as HTMLElement).contentEditable ==
        'false') &&
...
export function findCompositionNode(
  ...
  if (textAfter && textBefore && textAfter.node != textBefore.node) {
    ...
    if (
      !tileAfter ||
      (tileAfter.isText() && tileAfter.text != textAfter.node.nodeValue)
    ) {
      textNode = textAfter;
      ...
      if (
        !(
          !tileBefore ||
          (tileBefore.isText() && tileBefore.text != textBefore.node.nodeValue)
        )
      )
        textNode = textAfter;
    }
...
function nextToUneditable(node: Node, offset: number) {
  ...
  return (
    ...
    (node.childNodes[offset] as any).contentEditable == 'false'
      ? NextTo.After
      : 0)
  );
}
...
function findChangedWrappers(
  a: readonly RangeSet<BlockWrapper>[],
  b: readonly RangeSet<BlockWrapper>[],
  diff: ChangeSet,
) {
...
function touchesComposition(
  changes: ChangeSet,
  composition: null | { from: number; to: number },
) {
  let touched = false;
  if (composition)
