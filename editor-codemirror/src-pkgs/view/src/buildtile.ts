...
class TileBuilder {
  ...
  addText(
    ...
    if (
      prev &&
      prev.isText() &&
      !(prev.flags & TileFlag.Composition) &&
      prev.length + text.length < C.Chunk
    ) {
      this.cache.reused.set(prev, Reused.DOM);
      let tile = (parent.children[parent.children.length - 1] = new TextTile(
        prev.dom,
        prev.text + text,
      ));
      tile.parent = parent;
  ...
  addComposition(
    ...
    context: { marks: MarkTile[]; line: LineTile },
  ) {
    let line = this.curLine!;
    if (line.dom != context.line.dom) {
      line.setDOM(
        this.cache.reused.has(context.line)
          ? freeNode(context.line.dom)
          : context.line.dom,
      );
  ...
  addInlineWidget(
    ...
    let noSpace =
      this.afterWidget &&
      widget.flags & TileFlag.PointWidget &&
      (this.afterWidget.flags & TileFlag.PointWidget) ==
        (widget.flags & TileFlag.PointWidget);
    if (!noSpace) this.flushBuffer();
    let parent = this.ensureMarks(marks, openStart);
    if (!noSpace && !(widget.flags & TileFlag.Before))
  ...
  addLineStart(attrs: Attrs | null, dom?: HTMLElement) {
    ...
    let tile = LineTile.start(
      attrs,
      dom || this.cache.find(LineTile)?.dom,
      !!dom,
    );
  ...
  ensureMarks(marks: MarkDecoration[], openStart: number) {
    ...
    for (let i = marks.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        last instanceof MarkTile &&
        last.mark.eq(mark)
      ) {
        parent = last;
        openStart--;
      } else {
        let tile = MarkTile.of(
          mark,
          this.cache.find(MarkTile, (m) => m.mark.eq(mark))?.dom,
        );
        parent.append(tile);
        parent = tile;
        openStart = 0;
  ...
  endLine() {
    if (this.curLine) {
      ...
      if (
        !last ||
        !hasContent(this.curLine, false) ||
        (last.dom.nodeName != 'BR' &&
          last.isWidget() &&
          !(browser.ios && hasContent(this.curLine, true)))
      )
        this.curLine.append(
          this.cache.findWidget(BreakWidget, 0, TileFlag.After) ||
  ...
  updateBlockWrappers() {
    ...
    for (
      let cur = this.blockWrappers;
      cur.value && cur.from <= this.pos;
      cur.next()
    )
      if (cur.to >= this.pos) {
        ...
        while (
          i > 0 &&
          (this.wrappers[i - 1].rank - wrap.rank ||
            this.wrappers[i - 1].to - wrap.to) < 0
        )
          i--;
  ...
  getBlockPos() {
    ...
    for (let wrap of this.wrappers) {
      ...
      if (
        wrap.from < this.pos &&
        last instanceof BlockWrapperTile &&
        last.wrapper.eq(wrap.wrapper)
      ) {
        parent = last;
      } else {
        let tile = BlockWrapperTile.of(
          wrap.wrapper,
          this.cache.find(BlockWrapperTile, (t) => t.wrapper.eq(wrap.wrapper))
            ?.dom,
        );
        parent.append(tile);
        parent = tile;
  ...
  blockPosCovered() {
    ...
    return (
      ...
      !last.breakAfter &&
      (!last.isWidget() ||
        (last.flags & (TileFlag.After | TileFlag.IncEnd)) > 0)
    );
  }
...
class TextStream {
  ...
  next(maxLen: number): null | string {
    ...
    if (widgets.length)
      for (let i = 0, pass = 0; ; i++) {
        ...
        if (
          ...
          (pass == 0
            ? tile.widget.compare(widget)
            : tile.widget.constructor == widget.constructor &&
              widget.updateDOM(tile.dom, this.view, tile.widget))
        ) {
          widgets.splice(i, 1);
          ...
          if (
            tile.widget == widget &&
            tile.length == length &&
            (tile.flags & (TileFlag.Widget | TileFlag.BreakAfter)) == flags
          ) {
...
export class TileUpdate {
  ...
  constructor(
    ...
    this.builder = new TileBuilder(
      this.cache,
      new DocTile(view, view.contentDOM),
      RangeSet.iter(blockWrappers),
    );
  ...
  run(changes: readonly ChangedRange[], composition: Composition | null) {
    ...
    LOG_builder &&
      ...
      console.log(
        'Composition=',
        JSON.stringify(composition.range),
        composition.text.nodeValue,
      );
    LOG_builder && console.log('<<< ' + this.old.tile);
    let compositionContext =
      composition && this.getCompositionContext(composition.text);
    ...
    for (let posA = 0, posB = 0, i = 0; ; ) {
      ...
      if (
        composition &&
        next.fromA <= composition.range.fromA &&
        next.toA >= composition.range.toA
      ) {
        LOG_builder &&
          console.log(
            'Emit composition',
        ...
        this.forward(
          next.fromA,
          composition.range.fromA,
          composition.range.fromA < composition.range.toA ? 1 : -1,
        );
      ...
      skip: (tile, from, to) => {
        if (tile.isWidget()) {
          if (this.openWidget) {
            ...
            let widget =
              to > 0 || from < tile.length
                ? WidgetTile.of(
                    tile.widget,
                    this.view,
                    to - from,
                    tile.flags & TileFlag.Widget,
                    this.cache.maybeReuse(tile),
                  )
      ...
      point: (from, to, deco, active: MarkDecoration[], openStart, index) => {
        if (deco instanceof PointDecoration) {
          ...
          if (openStart > active.length) {
            ...
            let widget =
              deco.widget ||
              (deco.block ? NullWidget.block : NullWidget.inline);
            let flags = widgetFlags(deco);
            let tile =
              this.cache.findWidget(widget, to - from, flags) ||
              WidgetTile.of(widget, this.view, to - from, flags);
            if (deco.block) {
              if (deco.startSide > 0)
  ...
  getCompositionContext(text: Text) {
    ...
    for (
      let parent = text.parentNode as HTMLElement;
      ;
...
function widgetFlags(deco: PointDecoration) {
  let flags = deco.isReplace
    ? (deco.startSide < 0 ? TileFlag.IncStart : 0) |
      (deco.endSide > 0 ? TileFlag.IncEnd : 0)
    : deco.startSide > 0
      ? TileFlag.After
      : TileFlag.Before;
