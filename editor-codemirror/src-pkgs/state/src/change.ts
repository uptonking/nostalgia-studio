...
export class ChangeDesc {
  ...
  /// @internal
  static create(sections: readonly number[]) {
    return new ChangeDesc(sections);
  }
}
...
export class ChangeSet extends ChangeDesc {
  ...
  filter(ranges: readonly number[]) {
    let resultSections: number[] = [];
    let resultInserted: Text[] = [];
    let filteredSections: number[] = [];
  ...
  toJSON(): any {
    ...
    for (let i = 0; i < this.sections.length; i += 2) {
      ...
      if (ins < 0) parts.push(len);
        ...
        parts.push(
          ([len] as [number, ...string[]]).concat(
            this.inserted[i >> 1].toJSON(),
          ) as any,
        );
    }
    return parts;
...
class SectionIter {
  ...
  get text() {
    let { inserted } = this.set as ChangeSet;
    let index = (this.i - 2) >> 1;
