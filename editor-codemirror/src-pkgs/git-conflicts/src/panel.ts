...
import type { ConflictWidget } from './widgets';
...
class ConflictPanel {
    ...
    this.acceptBase = elt(
      ...
      elt(
        'button',
        {
          ...
          onclick: () => acceptAllConflicts(view, 'base'),
          role: 'menuitem',
          tabindex: '-1',
        },
    ...
    this.dom = elt(
        ...
        onkeydown: (event: KeyboardEvent) => {
          if (event.key == 'ArrowLeft' || event.key == 'ArrowRight') {
            const items = Array.from(
              this.dom.querySelectorAll('[role=menuitem]'),
            ) as HTMLElement[];
            const index = items.indexOf(
              this.dom.ownerDocument.activeElement as HTMLElement,
            );
            if (index < 0) items[0].focus();
            else if (event.key == 'ArrowLeft')
              items[index ? index - 1 : items.length - 1].focus();
      ...
      elt(
        ...
        elt(
          ...
          {
            class: 'cm-pseudo-link',
            onclick: () => acceptAllConflicts(view, 'ours'),
            role: 'menuitem',
          },
        ...
        elt(
          ...
          {
            class: 'cm-pseudo-link',
            onclick: () => acceptAllConflicts(view, 'theirs'),
            role: 'menuitem',
            tabindex: '-1',
          },
        ...
        elt(
          'button',
          {
            ...
            onclick: () => deleteAllConflicts(view),
            'aria-hidden': 'true',
            tabindex: '-1',
          },
          '×',
      ...
      elt(
        ...
        elt(
          'button',
          {
            ...
            onclick: () => selectFirstConflict(view),
            role: 'menuitem',
            tabindex: '-1',
        ...
        elt(
          ...
          {
            class: 'cm-text-button',
            onclick: () => selectPrevConflict(view),
            role: 'menuitem',
            tabindex: '-1',
        ...
        elt(
          ...
          {
            class: 'cm-text-button',
            onclick: () => selectNextConflict(view),
            role: 'menuitem',
            tabindex: '-1',
        ...
        elt(
          ...
          {
            class: 'cm-text-button',
            onclick: () => selectLastConflict(view),
            role: 'menuitem',
            tabindex: '-1',
  ...
  updateFor(state: EditorState) {
    const current = state.field(conflicts);
    if (current == this.current) return;
    this.current = current;
    const count = current.size;
    this.count.textContent = state.phrase(
      count == 1 ? '$ unresolved conflict' : '$ unresolved conflicts',
      count,
    );
    let hasBase = false;
  ...
  static create(view: EditorView) {
    return new ConflictPanel(view);
  }
}

export const conflictPanel = showPanel.compute([conflicts], (state) =>
  state.field(conflicts).size ? ConflictPanel.create : null,
);
