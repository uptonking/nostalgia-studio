...
const OptionControl: React.FC<IOptionControlProps> = ({
  option,
  onNameChange,
  onColorChange,
  onRemove,
  ...
  const { listeners, attributes, setNodeRef, transform, transition } =
    useSortable({ id: option.key as string });
...
export const SelectFieldControl: React.FC<ISelectFieldControlProps> = ({
  onChange,
  value,
}) => {
          ...
          <SortableContext items={items}>
            {options.map((o, index) => (
              <OptionControl
                ...
                option={o}
                onNameChange={(name) =>
                  handlers.setItemProp(index, 'name', name)
                }
                onColorChange={(color) =>
                  handlers.setItemProp(index, 'color', color)
                }
