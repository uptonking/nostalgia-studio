...
export const UpdateRecordForm: React.FC<IProps> = ({ onSuccess, onCancel }) => {
  ...
  const fields = columnOrder
    .map((fieldId) => schema.get(fieldId))
    .filter(Boolean);
  const { recordId } = useParams();

  const [
    updateRecord,
    { isLoading, isError, error, reset: resetUpdateRecord },
  ...
  return (
    <Box pt='sm'>
      <form onSubmit={onSubmit}>
        <Stack>
          {fields.map((field) => {
            const name: FieldPath<IMutateRecordValueSchema> = field.id
              .value as never;
            return (
              <RecordInputFactory
                name={name}
