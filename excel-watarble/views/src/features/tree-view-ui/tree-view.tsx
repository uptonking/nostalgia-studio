...
export const TreeView: React.FC<IProps> = ({
      ...
      >
        ...
        >
          ...
          {rowVirtualizer.getVirtualItems().map((virtualRow) => {
            const { id, children, values, collapsed, depth } =
              flattenedItems[virtualRow.index];
            return (
              <SortableTreeItem
                field={field}
                values={values}
  ...
  function handleDragEnd({ active, over }: DragEndEvent) {
    ...
    if (projected && over) {
      ...
      const clonedItems: FlattenedSortableRecord[] =
        flattenTree(items).map(cloneFlattened);
    ...
    if (overId && projected) {
      ...
      if (!previousItem) {
        ...
        if (projected.depth > previousItem.depth) {
          ...
          let previousSibling: FlattenedSortableRecord | undefined =
            previousItem;
