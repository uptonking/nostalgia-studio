      ...
      case 'object': {
        ...
        if (isArray) {
          ...
          while (++i < len) {
            const objKey = arr[i];
            result +=
              toIndexableString(objKey) + toIndexableString(key[objKey]);
          }
        }
        return result;
      }
...
function numToIndexableString(num) {
  ...
  const magString = padLeft(magForComparison.toString(), '0', MAGNITUDE_DIGITS);
