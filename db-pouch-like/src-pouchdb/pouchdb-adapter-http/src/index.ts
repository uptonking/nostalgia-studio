...
import { AbortController, fetch, Headers } from 'pouchdb-fetch';
import {
...
function HttpPouch(opts, callback) {
  ...
  api.get = adapterFun('get', async function (id, opts, callback) {
    ...
    function fetchAttachments(doc) {
      ...
      async function fetchData(filename) {
        ...
        let data;
        if (opts.binary) {
          const typeFieldDescriptor = Object.getOwnPropertyDescriptor(
            blob.__proto__,
            'type',
          );
  ...
  api.remove = adapterFun(
    'remove',
    async function (docOrId, optsOrRev, opts, cb) {
      let doc;
  ...
  // Get the attachment
  api.getAttachment = adapterFun(
    'getAttachment',
    async function (docId, attachmentId, opts, callback) {
      if (typeof opts === 'function') {
        callback = opts;
        opts = {};
      }
  ...
  api.removeAttachment = adapterFun(
    'removeAttachment',
    async function (docId, attachmentId, rev, callback) {
      const url =
        genDBUrl(
          host,
  ...
  api.putAttachment = adapterFun(
    'putAttachment',
    async function (docId, attachmentId, rev, blob, type, callback) {
  ...
  api._changes = function (opts) {
    ...
    if (opts.query_params && typeof opts.query_params === 'object') {
      for (const param_name in opts.query_params) {
        /* istanbul ignore else */
        if (Object.hasOwn(opts.query_params, param_name)) {
    ...
    const results: Record<string, any> = { results: [] };
