...
class Sync extends EE {
  canceled: boolean;
  push: ReturnType<typeof replicate>;
  pull: ReturnType<typeof replicate>;
  pushPaused: boolean;
  pullPaused: boolean;
  then: (success: any, err: any) => Promise<void | { push: any; pull: any }>;
  catch: (err: any) => Promise<void | { push: any; pull: any }>;
  constructor(src, target, opts, callback) {
    ...
    const removeAll = (type) => {
      ...
      return (event, func) => {
        const isChange =
          event === 'change' && (func === pullChange || func === pushChange);
        const isDenied =
          event === 'denied' && (func === pullDenied || func === pushDenied);
        const isPaused =
          event === 'paused' && (func === pullPaused || func === pushPaused);
        const isActive =
          event === 'active' && (func === pullActive || func === pushActive);
    ...
    const promise = Promise.all([this.push, this.pull]).then(
      (resp) => {
        const out = {
          push: resp[0],
          pull: resp[1],
