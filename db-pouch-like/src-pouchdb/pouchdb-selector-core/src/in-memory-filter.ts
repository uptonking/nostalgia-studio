...
function matchSelector(matcher, doc, parsedField, docFieldValue) {
  ...
  if (typeof matcher === 'object') {
    return Object.keys(matcher).every(function (maybeUserOperator) {
      const userValue = matcher[maybeUserOperator];
      // explicit operator
      if (maybeUserOperator.indexOf('$') === 0) {
        ...
        const subParsedField = parseField(maybeUserOperator);
        ...
        const subDocFieldValue = getFieldFromDoc(docFieldValue, subParsedField);
...
function modField(docFieldValue: string, userValue) {
  if (
...
var matchers = {
  ...
  $in: function (doc, userValue, parsedField, docFieldValue) {
    return (
      fieldExists(docFieldValue) && arrayContainsValue(docFieldValue, userValue)
    );
  },
