import { compare, getKey } from 'pouchdb-selector-core';
...
import { getUserFields } from './local-utils';
...
function getDefaultQueryPlan(selector, index?) {
...
function planQuery(request, indexes) {
  ...
  if (shouldShortCircuit(selector)) {
    return { ...SHORT_CIRCUIT_QUERY, index: indexes[0] };
  }
  ...
  const index = findBestMatchingIndex(
    selector,
    userFields,
    sortOrder,
    indexes,
  ...
  const inMemoryFields = getInMemoryFields(
    coreInMemoryFields,
    index,
    selector,
    userFields,
