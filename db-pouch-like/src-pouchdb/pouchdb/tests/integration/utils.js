...
function parseHostWithCreds(host) {
  const uriObj = testUtils.parseUri(host);
  const url = `${uriObj.protocol}://${uriObj.host}:${uriObj.port}${uriObj.path}`;
  const options = {};
  if (uriObj.userInfo) {
    options.headers = {};
    options.headers['Authorization'] =
      'Basic: ' + testUtils.btoa(uriObj.userInfo);
  }
  return { url, options };
}
...
testUtils.makeBlob = function (data, type) {
  if (testUtils.isNode()) {
    ...
    return globalThis.Buffer.from(data, 'binary');
  } else {
