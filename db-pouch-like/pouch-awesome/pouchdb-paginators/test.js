...
const POUCH_PATH = COUCH_URL ? `${COUCH_URL}/pouchdb-paginators-test` : '.test';
...
describe(name, function () {
  ...
  describe('allDocs', function () {
    it('should paginate ok', async function () {
      ...
      for await (const page of pager.pages()) {
        assert.equal(
          typeof page.rows.length,
          'number',
          'results did not resemble normal query results',
        );
        ...
        for (const row of page.rows) {
          ...
          assert(
            !(row.id in ids),
            'encountered duplicate id while paging forward',
          );
          ids[row.id] = true;
        }
      }
      ...
      for await (const page of pager.reverse()) {
        ...
        for (const row of page.rows) {
          ...
          assert(
            row.id in ids,
            'encountered duplicate id while paging backward',
          );
          delete ids[row.id];
        }
      }
      assert.equal(
        pager.hasPrevPage,
        false,
        'did not page through all pages while paging backwards',
      );
      assert.equal(
        total,
        NUM_DOCS,
        'did not page through all docs while paging backwards',
      );
    });
  ...
  describe('query', function () {
    ...
    it('should handle endkeys ok', async function () {
      ...
      assert.equal(
        total,
        Math.floor(NUM_DOCS / 2),
        'did not page through all docs',
      );
      // test reverse pagination
      total = 0;
      for await (const page of pager.reverse()) {
    ...
    it('should get the same page ok', async function () {
      ...
      for (let i = 0; i < page1.rows.length; i++) {
        ...
        assert.equal(
          row1.id,
          row2.id,
          'getting the same page again returned unexpected results',
        );
      }
    });
    ...
    describe('find', function () {
      ...
      it('should page through results ok', async function () {
        ...
        assert.equal(
          pager.hasNextPage,
          false,
          'did not page through all pages',
        );
        assert.equal(
          total,
          Math.floor(NUM_DOCS / 2),
